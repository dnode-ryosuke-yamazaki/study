...
1. トリガー: **OneDrive for Business「ファイルが作成されたとき」**、対象フォルダに `00_root/auto/teamsNotice/minutesNotice` を指定し、「ファイル コンテンツを含める」をはいにする。**`auto/teamsNotice/` を監視する既存フローがある場合は、その「サブフォルダーを含める」がオフであることを確認する**(オンだと議事録ファイルが既存フローにも検知され二重投稿になる)
