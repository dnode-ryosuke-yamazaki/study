...
flowchart LR
    ...
    uc1(["新規トランスクリプトを検知する"])
    uc2(["議事録Markdownを生成しOneDriveとローカル控えへ保存する"])
    uc3(["要約部分を投稿用に書き出す"])
    ...
    flow --> uc4
    user --> uc5
    tool["下流のツール<br>議事録TODOのJIRA起票"] --> uc6
    uc2 -- 控えを供給する --> uc6
```
...
- [6] 議事録をOneDriveの議事録フォルダへ保存するのと同時に、OneDriveの同期フォルダの外にあるローカルフォルダへも同じ内容を書き出すこと(下記[議事録のローカル控え](#議事録のローカル控え))
...
- [1] 議事録の控えは、OneDriveの同期フォルダの外に置くこと。根拠: OneDriveの同期フォルダは項目ごとにアプリへのアクセス許可が必要で、許可の無いプロセスからは議事録を読み取れない(このMacで実機確認済み。許可の付与は端末の管理設定により行えない)。議事録を入力に使う下流のツールが読める場所が必要
- [2] 控えのファイル名はOneDrive側と同じにすること。根拠: 下流のツールが同じ名前で会議を識別できるようにする(名前が変わると同じ議事録を別物と扱ってしまう)
- [3] 控えの書き出しに失敗しても、議事録の保存とTeamsへの共有は続けること。失敗は記録に残すこと。根拠: 控えは下流ツール向けの補助であり、これを理由に本来の成果物を落とさない
- [4] OneDrive側にあって控えに無い議事録は、読み取れる範囲で控えへ写すこと。読み取れないものは記録に残して飛ばすこと。根拠: 控えに無い議事録が存在しうるため([3]で控えの書き出しに失敗した分、控えの仕組みより前に生成された分、控えを手で消した分)。これにより[3]で落ちた控えは次の実行で回復する
- [5] 控えの内容はOneDrive側の議事録と同一とすること(要約や抜粋にしない)。根拠: 下流のツールが全文を前提に読む
- [6] 議事録の保存時に控えへ書き出す際、控えに同じ名前のファイルが既にある場合は上書きすること(控えに無い議事録を写す[4]の場合は、既にある控えに触れない)。根拠: 控えはOneDrive側の写しであり、名前の対応を保つことを優先する(控え側で別の連番を付けるとOneDrive側に存在しない名前が生まれ、下流のツールが同じ会議を別物として扱う)。控え側で履歴を残すことは目的ではない
...
- 会議ごとの投稿先チャネルの切り替え
- ローカル控えの世代管理・自動削除(テキストのため溜まっても問題にならない範囲とする)
