...
### 15. 動作中のロックが奪われないようにする

- ロックの更新時刻を今にできること。ロックが消えていても例外で止まらないこと
- バッチ本体が1件処理するごとに、成否によらずロックの時刻を更新すること

### 16. 議事録のローカル控え(config.py・writer.py)

- 控えフォルダのパスが状態フォルダの配下に導かれること。環境変数で差し替えられること
- `議事録を保存する` が、OneDrive側と同じファイル名で控えにも同じ内容を書くこと
- OneDrive側で連番が付いた場合、控えも同じ連番の名前になること
- 控えに同じ名前のファイルがある場合は上書きすること(requirements.md#議事録のローカル控え [6])
- 控えの書き出しが失敗しても例外を投げず、失敗した旨を呼び出し元に返すこと(議事録の保存とTeams共有を止めない)
- 対応: requirements.md#議事録の生成 [6] / requirements.md#議事録のローカル控え [1][2][3][5][6]

### 17. 控えのバックフィル(writer.py・generate_minutes.py)

- OneDrive側にあって控えに無い議事録だけを写すこと。既にあるものは触らないこと
- 議事録フォルダの一覧取得が権限で失敗した場合、例外にせず「読めなかった」として返すこと
- 個別のファイルが読めない場合はそのファイルを飛ばし、飛ばした件数を返すこと
- バッチ本体が、ロックを取得できた実行で1回だけ呼ぶこと(VTTごとに呼ばないこと)。状態ファイルが無い初回実行でも呼ぶこと(design.md#定期実行と未処理VTTの検知 手順3〜4)
- ログに写した件数・飛ばした件数を残すこと(本文・会議名は出さない)
- 対応: requirements.md#議事録のローカル控え [4] / design.md#控えのバックフィル

### 18. 運用ドキュメントの追記(README.md、テスト対象外)
