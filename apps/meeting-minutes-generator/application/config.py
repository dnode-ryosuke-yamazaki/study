...
#: 状態ファイル・ロック・ログの置き場。**同期フォルダの外**に置く
#: (同期フォルダに置くとOneDriveが競合ファイルを作り状態が二重化するため。
#: teams-transcript-fetcherと同じ方針)。仕様: design.md#状態管理
状態フォルダ = Path.home() / "Library/Application Support/meeting-minutes-generator"
...
class 設定:
    ...
    def ロックファイル(self) -> Path:
        return 状態フォルダ / "minutes.lock"

    @property
    def ログファイル(self) -> Path:
        return 状態フォルダ / "minutes.log"
