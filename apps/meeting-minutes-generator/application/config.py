...
#: 控えフォルダを差し替える環境変数。下流のツールが読む場所を変えたい場合のため。
控えフォルダ環境変数 = "MINUTES_GENERATOR_MIRROR_DIR"
...
class 設定:
    ...
    #: 議事録のローカル控えの置き場。**同期フォルダの外**に置く。OneDriveの同期
    #: フォルダは、macOSが項目ごとに管理するアプリへのアクセス許可が付いた項目しか
    #: 読めない(許可の付与は端末の管理設定により行えない)ため、議事録を入力に使う
    #: 下流のツールはこの控えを読む。仕様: requirements.md#議事録のローカル控え [1]
    控えフォルダ: Path
...
def load(
    作業フォルダ: Path | None = None,
    状態フォルダ: Path | None = None,
    控えフォルダ: Path | None = None,
) -> 設定:
    """設定を組み立てる。

    各フォルダは「引数 → 環境変数 → 既定値」の順で決める。引数を最優先にするのは、
    テストが実物の同期フォルダ・状態ファイル・控えに触れないようにするため。控えの
    既定値は状態フォルダの配下にし、状態フォルダを差し替えれば控えも一緒に移る。
    """
    if 作業フォルダ is None:
    ...
    if 控えフォルダ is None:
        控えの環境変数の値 = os.environ.get(控えフォルダ環境変数)
        控えフォルダ = (
            Path(控えの環境変数の値) if 控えの環境変数の値 else 状態フォルダ / "minutes"
        )
