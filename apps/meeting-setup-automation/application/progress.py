...
選択結果があれば代替案の2行には到達しないため、選択後に予定詳細ファイルを削除しても
来ないファイルを待ち続けない。
...
def 判定(設定値: 設定, 依頼id: str) -> 進行状態:
    ...
    予定詳細の依頼あり = ledger.予定詳細依頼ファイル(設定値, 依頼id).is_file()
    共通 = dict(試行番号=試行番号, 予定詳細の依頼あり=予定詳細の依頼あり, 代替案2の依頼あり=代替案2の依頼あり)
    ...
    if 予定詳細の依頼あり or 代替案2の依頼あり:
        予定詳細を待つ = 予定詳細の依頼あり and ledger.read_json(ledger.予定詳細ファイル(設定値, 依頼id)) is None
        代替案2を待つ = 代替案2の依頼あり and ledger.read_json(ledger.候補ファイル(設定値, 依頼id, 2)) is None
        return 進行状態(
            状態=代替案の提示中 if (予定詳細を待つ or 代替案2を待つ) else 代替案の提示済み,
            予定詳細を待つ=予定詳細を待つ, 代替案2の候補を待つ=代替案2を待つ, **共通,
        )
