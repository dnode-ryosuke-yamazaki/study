...
def ログを設定(設定値: config.設定) -> None:
    """作業フォルダのログファイル(5世代ローテーション)へ出す。標準エラーには異常だけを出し、標準出力はチャット向けの文に使う。
...
def _仮の予定の表示(w: candidates.枠, 名前表: Dict[str, str], 名簿: Optional[roster.名簿]) -> List[str]:
    """代替案1のカードに添える「仮の予定あり: <参加者名>」の1行。
    ...
    return [f"仮の予定あり: {'、'.join(名前一覧)}"] if 名前一覧 else []


def _待つ(env: 実行環境, 依頼id: str, 対象: Dict[str, tuple]) -> Dict[str, wait_for.待ち結果]:
    """待ちの開始をログに、進捗をチャット(標準出力)とログの両方に出す(design.md#ログ)。"""
...
def _作成結果を待つ(env: 実行環境, 依頼id: str, 再試行番号: int) -> int:
    対象名 = "作成結果" if 再試行番号 == 0 else f"作成結果(再試行{再試行番号})"
...
def cmd_resume(args, env: 実行環境) -> int:
    ...
    if 状態.状態 == progress.依頼済み:
        ...
        if 待ち.打ち切り:
            logger.warning("待ち上限で打ち切り: 依頼ID=%s 対象=候補(試行1) 経過=%d秒", 依頼id, 待ち.経過秒)
            env.出力(打ち切りの案内.replace("<依頼ID>", 依頼id))
            return 0
    ...
    if 状態.状態 == progress.代替案の提示中:
        if 状態.代替案2の候補を待つ:
            _待つ(env, 依頼id, {"代替案2の候補": (ledger.候補ファイル(設定値, 依頼id, 2), 設定値.候補待ち上限秒)})
