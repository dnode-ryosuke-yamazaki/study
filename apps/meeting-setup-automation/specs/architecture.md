...
flowchart TD
    subgraph cloud["Microsoft 365(クラウド)"]
        ...
        PA3["予定詳細フロー<br/>仮の予定の件名を取得"]
        PA4["会議設定通知フロー<br/>通知フォルダの新規ファイルをTeamsへ投稿"]
...
| Office 365 Outlookコネクタ | 予定表への到達手段。空き時間の探索・会議の作成・予定の状態と件名の取得に使う。**プレミアムコネクタ・カスタムコネクタは利用できない** |
...
|---|---|---|---|---|
| [meeting-scheduling](meeting-scheduling/requirements.md)([設計](meeting-scheduling/design.md)) | 参加者と所要時間を伝えるだけで候補が出て、選ぶだけでTeams会議が作られる | 空き時間から候補を提示し、選択された枠でTeams会議を作成して招待する | Power Automateフロー4本(探索フロー・作成フロー・予定詳細フロー・会議設定通知フロー)の新設、会議設定通知の投稿先の登録、OneDrive同期クライアントの稼働 | 仕様のみ(未実装) |
...
- **参加者の空き時間は他人の予定情報である。** 通常の候補では空いているかどうかだけを扱う。候補が0件のときの代替案1に限り、仮の予定に重ねてよいかの判断材料として予定の件名を取得して示すが、予定の本文と出席者は扱わず、非公開に設定された予定の件名も示さない。取得した件名は選択が済んだ時点で台帳から削除する

## 技術的制約

- **参加者の空き時間を返すコネクタは、時間帯や勤務時間で絞り込めない。** 返るのは純粋な空き時間で、時刻はすべて世界標準時。時間帯・曜日・祝日の絞り込みと日本時間への変換はすべてSkill側で行う
- **他人の予定の件名を読めるのは、開催者のカレンダー一覧にそのカレンダーが入っている参加者に限る。** 「イベントの取得 (V4)」はカレンダーを指定する形で、参加者のメールアドレスからは指定できない。カレンダーの一覧に並ぶのは表示名なので、名簿に表示名を持たせて突き合わせる。繰り返し予定は回ごとに展開されない
- **「イベントの取得 (V4)」が返す主催者の欄には接続アカウントが入り、実際の主催者ではない。** 主催者の判定には使えない
...
| 予定詳細 | 予定詳細フローが書き出す、指定した参加者の予定の状態と件名を表すファイル。候補が0件のときの代替案1でだけ使う |
...
| メンバー名簿 | 参加者の名前とメールアドレスの対応表。カレンダーの表示名を任意で持てる。Git管理外のローカル設定ファイル |
