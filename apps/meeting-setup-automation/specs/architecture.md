...
- **GraphパススルーのURIは「1階層目 + 2階層目」のパスしか受け付けず、クエリ文字列も書けない。** `me/calendar/getSchedule`のような関数の呼び出し、`me/events?$select=...`のようなクエリ付きのパスは、いずれもコネクタの検証で`400`になりGraphへ届かない(公式ドキュメントは「クエリはURIに直接書く」としているが実装は受け付けない)。絞り込みや並べ替えが必要な取得は、パススルーではなくコネクタの専用アクション(`Filter Query`・`Top Count`の入力を持つもの)で行う
