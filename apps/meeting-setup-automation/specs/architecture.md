...
- **Power Automateのフォルダ監視トリガーには検知の待ちがある。** OneDrive同期のラグと合わせて、依頼から候補の到着までに45〜70秒かかる前提で設計する。**この検知間隔は短時間に連続して依頼を出すと絞られ、往復が10分以上に伸びることがある**(実測: 7分間隔での連続実行で670秒)。連続して依頼を出す処理は、待ちが打ち切られても壊れない作りにする
