...
- **手順**:
  ...
  2. 参加者の名前をメンバー名簿でメールアドレスに解決する。照合は表記のゆれを吸収し、姓と名の区切りの違い・敬称の有無を同じ名前として扱い、姓だけ・名だけの指定も受け付ける。該当が1人に定まらない名前が1つでもある場合は、依頼を書き出さずに聞き返して終了する(部分的に解決できた分だけで先に進まない)。聞き返しでは、登録が無い名前はその旨を、複数人が該当する名前は該当者のフルネームとメールアドレスの一覧を示す
