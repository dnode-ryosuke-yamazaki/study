...
- **対象**: 絞り込みの結果が0件だった依頼
- **手順**:
  1. まだ緩められる項目があるかを調べる。時間帯を緩められるのは「開催者が時間帯を指定しておらず、依頼ファイルの時間帯が既定値と同じで、かつその実行でまだ絞り直していない」とき、期間を緩められるのは「開催者が探索期間を指定しておらず、試行番号が1」のとき。時間帯の絞り直しは1回の起動につき1回だけ行い、2度目はここへ戻ってきても期間の緩和へ進む
  ...
  8. どこまで緩めたかを別に記録しない。判断は依頼ファイルと試行番号、および試行番号1での絞り直しの有無から導ける([データ設計台帳ファイル](#データ設計台帳ファイル)の依頼ファイルの区分)。打ち切った後に別のセッションで再開しても同じ判定・同じ表示になる
