...
- **手順**:
  ...
  3. 代替案1で枠が残った場合は、その枠に仮の予定を持つ参加者の予定の件名を取りに行く。採用した枠を含む範囲と対象の参加者を予定詳細の依頼として書き出し、予定詳細フローの結果を待つ
...
- **依頼の識別** — 依頼ID
- **取得の指示(フローが読む)** — 対象の参加者のメールアドレス、取得する範囲の開始と終了、枠の刻み
...
件名は相手の空き時間の共有設定によって空で返ることがあり、非公開に設定された予定は件名を示さない。Skillは枠と時間が重なる仮の予定を選んで件名を拾う。
...
| 予定詳細フロー | 予定詳細の依頼フォルダにファイルが作成されたとき | 指示された参加者と範囲で`POST /me/calendar/getSchedule`を呼び、参加者ごとの予定と失敗理由を予定詳細ファイルとして書き出す。予定詳細のファイル名は、読んだ依頼のファイル名の `detail-request-` を `detail-` に置き換えて決める |
...
`POST /me/events`と`POST /me/calendar/getSchedule`はOffice 365 Outlookコネクタのパススルー経由で呼ぶ。会議オプション画面の直リンクは応答の会議本文から取り出す必要があり、本文を返す経路を選ぶ。
...
| 候補が0件のときの代替案の提示-3 | 単体 | 開催者が指定した項目を広げないこと。両方指定されていれば代替案2を作らないこと |
| 候補が0件のときの代替案の提示-4 | 単体 | どちらの代替案によるものかが分かる形で示すこと |
| 候補が0件のときの代替案の提示-5 | 単体 | 代替案からさらに緩めず、両方0件なら相談に切り替えること |
| 候補が0件のときの代替案の提示-6 | 単体 | 受け取った枠が要求件数と同数のときに打ち切りの可能性を伝えること |
| 候補が0件のときの代替案の提示-7 | 結合 | 代替案の提示は依頼1件につき1回。台帳の有無から判定でき、再開しても同じ結果になること |
