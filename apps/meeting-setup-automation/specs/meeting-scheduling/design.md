...
| 予定詳細フロー | 予定詳細の依頼フォルダにファイルが作成されたとき | 「予定表の取得 (V2)」でカレンダーの一覧を取り、指示された表示名に一致するものだけを対象に「イベントの取得 (V4)」を呼んで、参加者ごとの予定と失敗理由を予定詳細ファイルとして書き出す。一致するカレンダーが無い参加者は「件名を取得できない」として書き出す。予定詳細のファイル名は、読んだ依頼のファイル名の `detail-request-` を `detail-` に置き換えて決める |
...
**予定の取得にパススルーは使わない。** このコネクタのパススルーで通るのは`POST /me/events`だけで、`me/calendar/getSchedule`のような3階層のパスやクエリ付きのGETはコネクタの検証で弾かれる([architecture.md#技術的制約](../architecture.md#技術的制約))。代わりに専用アクションの「予定表の取得 (V2)」と「イベントの取得 (V4)」を使う。後者は`Calendar id`でカレンダーを指定する形で、参加者のメールアドレスからは指定できない。**「イベントの取得 (V4)」が返す主催者の欄には接続アカウントが入り、実際の主催者ではないため使わない。**
