...
| 候補到着 | 試行番号1の候補があり、代替案の台帳(予定詳細の依頼・試行番号2の依頼)がどちらも無く、選択結果が無い | 既定の条件で絞り込み・選択画面の用意・選択の受け取り |
| 代替案の提示中 | 代替案の台帳があり、そのうち求めた結果がまだ揃っておらず、選択結果が無い | 予定詳細と代替案2の候補の到着を待つ |
| 代替案の提示済み | 代替案の台帳があり、求めた結果が揃っており、選択結果が無い | 代替案1と代替案2の両方を絞り直して1枚の選択画面にまとめ、選択を受け取る |
...
**上の表は上から順に当てはめ、最初に一致した行をその依頼の状態とする。** 選択結果があれば代替案の2状態には当てはまらないため、予定詳細ファイルを選択後に削除しても([セキュリティ](#セキュリティ))、来ないファイルを待ち続けることはない。

下の図の「打ち切り」と「相談」は、上の表の状態ではなく、待ちをやめて終了した位置を表す。台帳ファイルから導けるのは表の7状態だけで、打ち切って終了した依頼も、緩められる項目がなくなって相談で終わった依頼も、台帳の上では直前の状態のまま残る。相談で終わった依頼を依頼IDを指定して再開した場合は、代替案の台帳が残っているため「代替案の提示済み」として同じ絞り直しを行い、同じ結論(相談)に至る。
