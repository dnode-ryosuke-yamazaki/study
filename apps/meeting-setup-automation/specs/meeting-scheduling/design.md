...
- **手順**:
  ...
  9. 両方の代替案とも0件の場合は、候補が見つからないことと試した3通りの条件、広げなかった項目(開催者が指定したため広げなかったこと)を挙げて、参加者・期間・所要時間のどれを緩めるかを開催者に相談して終了する
  10. 代替案の候補からさらに自動で条件を緩めない
  11. どこまで進んだかを別に記録しない。代替案を提示したかどうかは、予定詳細の依頼または試行番号2の依頼があるかで判定する([データ設計台帳ファイル](#データ設計台帳ファイル))。打ち切りや相談で終わった後に別のセッションで再開しても同じ判定・同じ表示になる
...
クリップボードへ入れる1行は、目印の語・依頼ID・試行番号・候補番号・開始時刻・終了時刻を並べたものにする。時刻は日本時間のオフセット付きで書く(台帳の日時と同じ書き方に揃え、貼られた側が時点として解釈できるようにする)。目印の語を先頭に置くことで、貼られた内容が選択結果かどうかを取り違えずに判定できる。
...
| 代替案の提示中 | 既定の絞り込みが0件で、予定詳細の依頼または試行番号2の依頼があり、その結果がまだ揃っていない | 予定詳細と代替案2の候補の到着を待つ |
...
下の図の「打ち切り」と「相談」は、上の表の状態ではなく、待ちをやめて終了した位置を表す。台帳ファイルから導けるのは表の6状態だけで、打ち切って終了した依頼も、緩められる項目がなくなって相談で終わった依頼も、台帳の上では直前の状態のまま残る。相談で終わった依頼を依頼IDを指定して再開した場合は、同じ候補を同じ条件で絞り直して同じ結論(相談)に至る。
...
stateDiagram-v2
    ...
    代替案の提示中 --> 候補到着: 代替案の枠が出た
    代替案の提示中 --> 相談: どちらの代替案も0件
