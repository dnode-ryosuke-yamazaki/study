...
- **手順**:
  ...
  2. 候補ファイルに失敗理由が入っている場合は、候補の絞り込みへ進まずその理由を伝えて終了する。ただし代替案2の候補ファイルの失敗は[候補が0件のときの代替案の提示](#候補が0件のときの代替案の提示)の手順4で扱い、ここでは終了させない
  ...
  12. 選択画面のURLと件名・候補件数を通知として通知フォルダへ書き出し、同じ内容をチャットにも表示する。代替案を提示する場合は、どちらの代替案によるものか、代替案1の枠の仮の予定を持つ参加者、代替案2の広げた後の条件も添える。通知文には、開けない場合は数十秒待って再読込する旨を添える
...
- **手順**:
  ...
  3. **代替案2(期間と時間帯を広げる)**: 期間を当日から3週間、時間帯を9時30分から18時30分に広げた依頼を次の試行番号で書き出し、その候補ファイルの到着を待つ。**開催者が明示的に指定した項目は広げない**。広げられる項目が1つもない場合は代替案2を作らない
  4. 手順3の候補には待ち上限を適用する。得られなかった場合は、代替案1の枠だけを提示し、代替案2はその理由を添えて伝える。**「得られなかった」には、待ち上限を超えた場合・結果ファイルに失敗理由が入っていた場合・依頼を書き出せなかった場合を含める**。代替案2の候補ファイルに失敗理由が入っていた場合も、[候補の絞り込みと選択画面の用意](#候補の絞り込みと選択画面の用意)の手順2の一般則(失敗理由があれば終了する)には従わず、代替案1の枠の提示を続ける(手元にある枠を出せないまま終わらせない)
  5. 代替案の依頼は既定の依頼から間を置かずに続くため、フォルダ監視トリガーの検知間隔が絞られて往復が伸び、待ち上限を超えて打ち切られることがある。これを異常として扱わず、打ち切りとして伝えて依頼IDでの再開に委ねる
  6. 2つの代替案の枠を1枚の選択画面にまとめ、どちらの代替案によるものか、代替案1の枠には仮の予定を持つ参加者、代替案2の枠には広げた後の条件を添えて示す
  7. 代替案1が0件で、かつ受け取った枠の件数が依頼で要求した最大件数と同数だった場合は、応答が件数で打ち切られている可能性も併せて伝える
  8. 両方の代替案とも0件の場合は、候補が見つからないことと試した条件(代替案2を作らなかった場合は既定と代替案1の2通り、作った場合は3通り)、広げなかった項目(開催者が指定したため広げなかったこと)を挙げて、参加者・期間・所要時間のどれを緩めるかを開催者に相談して終了する
  9. 代替案の候補からさらに自動で条件を緩めない
  10. どこまで進んだかを別に記録しない。代替案を提示したかどうかは、試行番号2の依頼があるかで判定する([データ設計台帳ファイル](#データ設計台帳ファイル))
  11. 代替案の台帳がある依頼を依頼IDを指定して再開した場合は、通常の絞り込みへ進まず「代替案の提示済み」として手順6から入り、代替案1と代替案2の両方を絞り直して1枚の選択画面にまとめ直す([状態管理](#状態管理))。試行番号2の候補だけを絞り込んで代替案1の枠を落とさない
...
- **フローは失敗しても結果ファイルを書く。** 探索フローと作成フローは、コネクタの呼び出しが失敗した場合も失敗理由を入れた結果ファイルを書き出す。Skillは結果ファイルに失敗理由があればその時点で理由を伝えて終了する(待ち上限まで待たされない)。ただし代替案の2つの結果は独立して扱い、片方の失敗でもう片方の提示を止めない
- **同期フォルダへの書き込み失敗はそこで止める。** 依頼・選択結果・通知・HTMLのいずれも、書き出しに失敗したら次の手順へ進まない。ただし代替案2の依頼の書き出し失敗は例外とし、代替案1の枠だけを提示して、代替案2は書き出せなかった旨を伝える(2つの代替案は独立しており、片方が作れなくても選択に進める)。通知の書き出し失敗も例外とし、チャットへの表示を行って処理を続ける(通知はOneDrive同期とフローに依存するため、そこが止まっても機能全体を使えなくしない)
...
| 候補の待ち・絞り込み・代替案の提示・選択画面の書き出し・同期の猶予待ち・ビューアURLの組み立て・通知の書き出し。および選択済みの依頼を再開したときの作成結果の待ちと完了の通知、作成失敗を検知したときの失敗理由の提示 | `main.py resume` |
...
- **代替案をすでに提示したか** — 試行番号2の依頼ファイルがあるかどうか
...
| 代替案の提示済み | 試行番号2の依頼があり、その候補が届いている | 代替案1と代替案2の両方を絞り直して1枚の選択画面にまとめ、選択を受け取る |
| 代替案の提示中 | 試行番号2の依頼があり、その候補がまだ届いていない | 代替案2の候補の到着を待つ |
...
「代替案の提示済み」で結果が揃っているかは、試行番号2の依頼に対応する候補があるかで見る。代替案2を作らなかった依頼では代替案の台帳が増えないため、再開したときは既定の絞り込みからやり直して同じ結果に至る。
...
- **他人の予定について扱うのは空き状況だけに限る。** 候補・代替案1のいずれでも、予定の件名・本文・出席者は取得も表示もしない。代替案1で示すのは「誰に仮の予定があるか」までとする。探索フローも空き状況以上のものを候補ファイルに書かない
...
| 候補が0件のときの代替案の提示-2 | 単体 | 代替案1の枠に仮の予定を持つ参加者を添えること。予定の件名を扱わないこと |
| 候補が0件のときの代替案の提示-3 | 単体 | 代替案1で示す参加者に開催者自身を含めること |
...
| 往復の待ち時間の上限-2 | 単体 | 代替案2の候補の待ちにも同じ上限を適用する |
