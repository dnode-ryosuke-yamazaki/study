...
- **手順**:
  ...
  3. 代替案1で枠が残った場合は、その枠に仮の予定を持つ参加者のうち、メンバー名簿にカレンダーの表示名が登録されている参加者について予定の件名を取りに行く。採用した枠を含む範囲と対象の参加者の表示名を予定詳細の依頼として書き出し、予定詳細フローの結果を待つ。表示名が登録されていない参加者は件名を取りに行かず、取得できないものとして扱う
...
- **依頼の識別** — 依頼ID
- **取得の指示(フローが読む)** — 対象の参加者のカレンダーの表示名と、対応するメールアドレス、取得する範囲の開始と終了
...
| 予定詳細フロー | 予定詳細の依頼フォルダにファイルが作成されたとき | 「カレンダーの取得 (V2)」でカレンダーの一覧を取り、指示された表示名に一致するものだけを対象に「イベントの取得 (V4)」を呼んで、参加者ごとの予定と失敗理由を予定詳細ファイルとして書き出す。一致するカレンダーが無い参加者は「件名を取得できない」として書き出す。予定詳細のファイル名は、読んだ依頼のファイル名の `detail-request-` を `detail-` に置き換えて決める |
...
**予定の取得にパススルーは使わない。** このコネクタのパススルーで通るのは`POST /me/events`だけで、`me/calendar/getSchedule`のような3階層のパスやクエリ付きのGETはコネクタの検証で弾かれる([architecture.md#技術的制約](../architecture.md#技術的制約))。代わりに専用アクションの「カレンダーの取得 (V2)」と「イベントの取得 (V4)」を使う。後者は`Calendar id`でカレンダーを指定する形で、参加者のメールアドレスからは指定できない。**「イベントの取得 (V4)」が返す主催者の欄には接続アカウントが入り、実際の主催者ではないため使わない。**
...
| 候補が0件のときの代替案の提示-3 | 単体 | 表示名が未登録の参加者・繰り返し予定・カレンダーが見つからない参加者を、件名を取得できないものとして扱うこと |
...
| 参加者の解決-4 | 単体 | 名簿のカレンダーの表示名を任意項目として読み、未登録なら件名の取得を試みないこと |
