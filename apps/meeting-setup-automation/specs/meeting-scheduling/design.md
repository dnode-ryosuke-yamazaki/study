...
2. **探索条件の判断はすべてSkillに置き、フローには「渡された条件で探す」だけをさせる。** 候補探索のコネクタは1つの期間しか受け取れないため、曜日・祝日・時間帯の絞り込みはSkill側で行う。フローには多めの件数を返させ、Skillが絞って上限件数に落とす([候補の絞り込みと選択画面の用意](#候補の絞り込みと選択画面の用意))
...
- **手順**:
  ...
  2. 時間帯が開催者の指定でなく、まだ広げていない場合は、時間帯を9時30分から18時30分に広げた依頼を次の試行番号で書き出し、[候補の絞り込みと選択画面の用意](#候補の絞り込みと選択画面の用意)へ戻る。対象とする曜日は変えない
  3. そうでなく、探索期間が開催者の指定でなく、まだ広げていない場合は、期間を当日から3週間に広げた依頼を次の試行番号で書き出し、同じく戻る。時間帯と対象とする曜日は前段の値のままにする
...
| 候補探索の既定条件-3 | 単体 | 全員空きの絞り込みと指示による例外。例外時に出席可能率の下限も併せて下げること |
...
| 往復の待ち時間の上限-2 | 手動 | 実測した往復時間から上限を決める。候補側は[tasks.md](tasks.md#仕様承認pr前に行う実機確認)のE1、作成側はE2で確認する |
