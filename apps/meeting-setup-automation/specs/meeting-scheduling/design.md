...
チャットから起動したSkillが、参加者名をメンバー名簿でメールアドレスに解決し、探索条件を添えた**依頼**をOneDriveの依頼フォルダへ書き出す。Power Automateフロー①が参加者全員の空き時間を探して**候補**を書き出し、Skillはそれを待って絞り込み、**候補選択画面**(静的HTML)をOneDriveへ置いてビューア形式URLをTeamsとチャットへ知らせる。開催者が画面で選んだ結果をチャットに貼ると、Skillが**選択結果**を書き出し、Power Automateフロー②がTeams会議を作成して**作成結果**を書き出す。Skillは作成結果から参加URLと会議オプション画面の直リンクを取り出して完了を通知する。

主要な設計判断は次の6点。

1. **依頼の進行状態は台帳ファイルの存在から導き、状態ファイルを持たない。** ファイル名に依頼IDを含めるため、どこまで進んだかは「どのファイルがあるか」で一意に決まる。待ちを打ち切った後の再開もこれだけで成り立つ([状態管理](#状態管理))
2. **探索条件の判断はすべてSkillに置き、フローには「渡された条件で探す」だけをさせる。** 候補探索のコネクタは1つの期間しか受け取れないため、平日・祝日・時間帯の絞り込みはSkill側で行う。フローには多めの件数を返させ、Skillが絞って上限件数に落とす([候補の絞り込みと選択画面の用意](#候補の絞り込みと選択画面の用意))
3. **フローは失敗しても必ず結果ファイルを書く。** 書かない設計だとSkillは待ち上限まで待つしかなく、失敗の理由も伝わらない。候補・作成結果のどちらも失敗理由を入れる区分を持つ([エラーハンドリング](#エラーハンドリング))
4. **選択結果ファイルは単体で会議を作れる内容にする。** フロー②が依頼ファイルを読みに行かなくて済むため、フローが「渡された枠で作る」だけの役に徹せる([データ設計台帳ファイル](#データ設計台帳ファイル))
...
- **手順**:
  1. 件名・参加者・所要時間・アジェンダを受け取る。探索の期間・時間帯・候補件数の指定があれば受け取り、指定のない項目には既定値(当日から2週間、平日9時30分から17時30分、最大5件)を使う
  ...
  5. 依頼を依頼フォルダへ書き出す。ファイル名には依頼IDと試行番号を含める。試行番号は初回が1で、候補が0件だったときの再探索ごとに1増える
...
sequenceDiagram
    ...
    participant R as メンバー名簿
    participant D as OneDrive 台帳フォルダ
    participant F1 as 探索フロー
    participant C as Exchange 予定表
    participant N as 会議設定通知フロー
    participant T as Teams チャネル
...
図の正となる文章は本見出しおよび[候補の絞り込みと選択画面の用意](#候補の絞り込みと選択画面の用意)。
...
- **手順**:
  ...
  3. 受け取った枠を、この依頼の条件で絞り込む。土曜・日曜・祝日にかかる枠を除く。枠の開始から終了までが指定の時間帯に収まらないものを除く。参加者全員が空いていない枠を除く(依頼時に一部不在の枠も含めるよう指示された場合はこの除外を行わない)。開始時刻の早い順に並べ、既に採用した枠と時間の重なる枠を除く
  4. 絞り込んだ結果から先頭を上限件数まで採用する。同じ日に複数の枠が残ってもそのまま採用する
  5. 採用した枠が0件の場合は[候補が0件のときの再探索](#候補が0件のときの再探索)へ進む
  6. 採用した枠から候補選択画面(静的HTML)を組み立て、HTMLフォルダへ置く。ファイル名には依頼IDを含める。同じ依頼IDの画面が既にある場合は上書きする(再探索で候補が入れ替わったときに古い画面を残さない)
  7. 置いたHTMLを読み戻し、書いた内容と一致すること、続けて読んだときにサイズが変わらないことを確認する
  8. 同期の猶予時間(既定30秒)だけ待つ
  9. HTMLのビューア形式URLを組み立てる。ビューアのURLに付随するクエリは落とし、サーバー相対パスは一度デコードしてからエンコードする(エンコード済みの値を二重にエンコードしないため)
  10. 選択画面のURLと件名・候補件数を通知として通知フォルダへ書き出し、同じ内容をチャットにも表示する。通知文には、開けない場合は数十秒待って再読込する旨を添える
  11. 通知の書き出しに失敗しても、チャットへのURL表示は行って選択の受け取りへ進む(通知だけの失敗で機能全体を止めない)
...
- **対象**: 絞り込みの結果が0件だった依頼
- **手順**:
  1. 試行番号が1の場合は、時間帯を平日9時30分から18時30分に広げた依頼を試行番号2で書き出し、[候補の絞り込みと選択画面の用意](#候補の絞り込みと選択画面の用意)へ戻る
  2. 試行番号が2の場合は、期間を当日から3週間に広げた依頼を試行番号3で書き出し、同じく戻る。時間帯は18時30分までのままにする
  3. 試行番号が3の場合は、それ以上広げない。候補が見つからないことと、試した3通りの条件を挙げて、参加者・期間・所要時間のどれを緩めるかを開催者に相談して終了する
  4. 再探索の待ちは新しい待ちとして扱い、待ちごとに待ち上限を適用する
...
- **手順**:
  ...
  2. 読み取った依頼IDの候補ファイルを開き、候補番号の枠が存在し、開始時刻・終了時刻が一致することを確認する。一致しない場合は書き出さずに聞き返す(古い画面や別の依頼の内容を貼った場合に取り違えないため)
  3. その依頼IDの選択結果ファイルが既にある場合は、新しく書き出さない。作成結果が既にあれば作成済みの内容をそのまま伝え、なければ作成結果の待ちへ進む
  4. アジェンダを見出しと箇条書きのHTMLに整形する
  ...
  7. 作成結果に失敗理由が入っている場合は、その理由を伝えて終了する(選択結果ファイルは残すため、フロー側の問題を直せば再実行できる)
...
- **手順**:
  ...
  4. 待ち上限(既定5分)を超えたら待ちを打ち切る。打ち切ったことと、依頼IDを指定して再度起動すれば続きから再開できることを伝えて終了する
...
- **二重の会議作成を防ぐ。** 選択結果ファイルは1つの依頼IDについて1回しか書き出さない。作成結果ファイルが既にある依頼IDについては選択結果を書き出さず、作成済みの内容を伝える
...
- `apps/meeting-setup-automation/application/request.py` — 依頼IDの生成、依頼の組み立てと書き出し、再探索の条件の拡張
- `apps/meeting-setup-automation/application/wait_for.py` — 台帳ファイルの到着待ちと進捗の出力
- `apps/meeting-setup-automation/application/candidates.py` — 候補の絞り込み(平日・祝日・時間帯・全員空き・重複除外)
...
- `apps/meeting-setup-automation/application/power-automate/README.md` — 探索フロー・作成フローの新設手順と設定内容
...
| 選択結果 | `auto/meetingSetting/selection/` | Skill | 作成フロー | `selection-<依頼ID>.json` |
| 作成結果 | `auto/meetingSetting/result/` | 作成フロー | Skill | `result-<依頼ID>.json` |
...
- 依頼IDは「起動時刻(年月日と時分秒)+乱数4文字」の形にする。試行番号は初回が1で、再探索ごとに1増える
...
- **依頼の識別** — 依頼IDと試行番号
- **会議の内容** — 件名・所要時間・必須出席者のメールアドレス
- **探索の指示(フローが読む)** — 探索期間の開始と終了、探索する活動の範囲、出席可能率の下限、返してほしい枠の最大件数
- **絞り込みの条件(Skillだけが読む)** — 時間帯の開始と終了、祝日を除くかどうか、最終的に提示する候補の上限件数

探索のコネクタは1つの期間しか受け取れないため、フローには上限件数より多めの枠を返させ、絞り込みの条件はSkillが持つ。絞り込みの条件を依頼ファイルに書いておくことで、待ちを打ち切った後の再開でも同じ条件で絞り込める(別の状態ファイルを持たない)。

### 候補ファイルの区分

- **依頼の識別** — 依頼IDと試行番号
- **枠の一覧** — 枠ごとの開始・終了、出席可能率、出席者ごとの空き状況
...
| フロー | トリガー | 行うこと |
|---|---|---|
| 探索フロー | 依頼フォルダにファイルが作成されたとき | 依頼の探索の指示を「会議の時間を検索 (V2)」へ渡し、返った枠と失敗理由を候補ファイルとして書き出す |
| 作成フロー | 選択結果フォルダにファイルが作成されたとき | 選択結果の内容で`POST /me/events`を呼んでオンライン会議を作成し、応答と失敗理由を作成結果ファイルとして書き出す |
| 会議設定通知フロー | 通知フォルダにファイルが作成されたとき | ファイルの内容を自分専用チャネルへ投稿する(構築済みの仕組みを使う) |
...
- ヘッダー: 件名・所要時間・参加者の人数・探索した条件の要約
- 候補カード(1候補につき1枚、縦に並べる): 候補番号・日付・曜日・開始と終了の時刻・参加者の空き状況。ラジオボタンで1つだけ選べる
...
| 選択済み | 選択結果があり、作成結果が無い | 作成結果の到着を待つ |
| 作成済み | 作成結果がある | 完了内容を伝える |

試行番号は、その依頼IDの依頼ファイルのうち最も大きい番号を今の試行とする。
...
stateDiagram-v2
    ...
    依頼済み --> 候補到着: 候補が届いた
    依頼済み --> 打ち切り: 待ち上限を超えた
    候補到着 --> 依頼済み: 絞り込みの結果が0件で再探索する
    候補到着 --> 相談: 再探索を使い切っても0件
...
| 再探索 | 依頼ID・新しい試行番号・広げた条件 | INFO |
| 再探索を使い切って0件 | 依頼ID・試した条件 | WARNING |
...
| 候補探索の既定条件-2 | 単体 | 土日・祝日の除外 |
| 候補探索の既定条件-3 | 単体 | 全員空きの絞り込みと指示による例外 |
| 候補探索の既定条件-4 | 単体 | 重なる枠の除外 |
| 候補が0件のときの探索条件の拡張-1 | 単体 | 時間帯の拡張 |
| 候補が0件のときの探索条件の拡張-2 | 単体 | 期間の拡張 |
| 候補が0件のときの探索条件の拡張-3 | 単体 | 相談に切り替える |
| 候補が0件のときの探索条件の拡張-4 | 単体 | 拡張は2回まで |
| 往復の待ち時間の上限-1 | 単体 | 上限5分 |
...
| 録画とファシリテーターの設定-3 | 手動 | 自前の本文を指定しても直リンクが残ること |
