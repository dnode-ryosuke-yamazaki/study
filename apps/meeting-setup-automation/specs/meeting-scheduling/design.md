...
- **手順**:
  ...
  3. 受け取った枠を、この依頼の条件で絞り込む。土曜・日曜・祝日にかかる枠を除く。対象とする曜日の指定がある場合はそれ以外の曜日の枠を除く。枠の開始から終了までが指定の時間帯に収まらないものを除く。参加者全員が空いていない枠を除く(依頼時に一部不在の枠も含めるよう指示された場合はこの除外を行わない)。開始時刻の早い順に並べ、既に採用した枠と時間の重なる枠を除く
...
| 候補の待ち・絞り込み・再探索・選択画面の書き出し・同期の猶予待ち・ビューアURLの組み立て・通知の書き出し・作成失敗を検知したときの失敗理由の提示 | `main.py resume` |
...
- 依頼IDは「起動時刻(年月日と時分秒)+乱数4文字」の形にする。試行番号は初回が1で、再探索ごとに1増える。再試行番号は初回の作成が0で、会議の作成をやり直すたびに1増える
...
- **絞り込みの条件(Skillだけが読む)** — 時間帯の開始と終了、祝日を除くかどうか、対象とする曜日、全員空きを求めるかどうか、最終的に提示する候補の上限件数
...
| 候補探索の既定条件-3 | 単体 | 全員空きの絞り込みと指示による例外。例外時に出席可能率の下限も併せて下げること、曜日の指定で絞り込めること |
...
| 往復の待ち時間の上限-2 | 手動 | 実測した往復時間から上限を決める。[tasks.md](tasks.md#仕様承認pr前に行う実機確認)のE1で確認する |
| 往復の待ち時間の上限-3 | 単体 | 待ちごとに上限を適用する |
