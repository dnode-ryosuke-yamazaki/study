...
**パススルーで通るのは`POST /me/events`だけ。** `me/calendar/getSchedule`のような3階層のパスやクエリ付きのGETはコネクタの検証で弾かれる([architecture.md#技術的制約](../architecture.md#技術的制約))。予定の読み取りはこの機能では行わない。
...
- 代替案として提示する場合は、代替案1の枠と代替案2の枠を見出しで分けて並べる。代替案1の枠には仮の予定を持つ参加者の名前を、代替案2の枠には広げた後の期間と時間帯を添える
- 参加者の空き状況は「全員空き」または空いていない人数だけを示す。予定の件名は示さない
