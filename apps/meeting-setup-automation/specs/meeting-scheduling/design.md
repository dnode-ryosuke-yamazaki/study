...
- **対象**: 絞り込みの結果が0件だった依頼
- **手順**:
  1. まだ緩められる項目があるかを調べる。時間帯を緩められるのは「開催者が時間帯を指定しておらず、依頼ファイルの時間帯が既定値と同じで、かつその実行でまだ絞り直していない」とき、期間を緩められるのは「開催者が探索期間を指定しておらず、試行番号が1」のとき。時間帯の絞り直しは1つの試行につき1回だけ行い、2度目はここへ戻ってきても期間の緩和へ進む
...
- **これから緩められるか**(緩和を進めるかの判断) — 時間帯は「開催者が時間帯を指定しておらず、依頼ファイルの時間帯が既定値と同じで、かつその実行でまだ絞り直していない」、期間は「開催者が探索期間を指定しておらず、試行番号が1」
- **すでに緩めたか**(選択画面と通知に緩めた後の条件を示すかの判断) — 時間帯は「開催者が時間帯を指定しておらず、かつ『依頼ファイルの時間帯が既定値と違う』または『その実行で絞り直した』」、期間は「開催者が探索期間を指定しておらず、試行番号が2以上」

時間帯の緩和は依頼を出し直さないため、試行番号1のあいだは依頼ファイルの時間帯が既定値のままになる。この1回だけは「その実行で絞り直した」で拾う。期間を広げた依頼には緩めた後の時間帯を書き込むため(手順3)、試行番号2以降は依頼ファイルの値から導ける。したがって打ち切った後に別のセッションで再開しても、同じ判定・同じ表示になる。
