...
7. **候補が0件のときは条件を段階的に緩めず、2つの代替案を並べて開催者に選ばせる。** 仮の予定を含めた候補と、期間・時間帯を広げた候補を同時に用意する。前者は受け取った候補の絞り直しだけで済み、後者は依頼を1件増やす。段階的に緩める方式だと往復のたびに待ちが発生し、どこまで緩んだのかも伝わりにくい([候補が0件のときの代替案の提示](#候補が0件のときの代替案の提示))
8. **会議の作成の失敗は完了と区別し、開催者の指示があったときだけやり直す。** 作成結果に失敗理由があれば「作成失敗」として扱い、再試行番号を増やした選択結果を書き出して作成フローを改めて発火させる。自動では再試行しない([会議の作成が失敗したときの再試行](#会議の作成が失敗したときの再試行))
9. **チャットでのやり取りはSKILL.mdが、ファイルの読み書きと判定は`main.py`が担う。** どちらでもできる手順の担い手を先に決めることで、同期の猶予待ちやチャットへの提示が実装から抜け落ちるのを防ぐ([手順の担い手](#手順の担い手))
...
- **手順**:
  ...
  2. 参加者の名前をメンバー名簿でメールアドレスに解決する。照合は表記のゆれを吸収し、姓と名の区切りの違い・敬称の有無を同じ名前として扱い、姓だけ・名だけの指定も受け付ける。打たれたとおりの照合を敬称を外した照合より先に見て、同じ照合ではフルネームとしての該当と姓・名としての該当を合わせて数える。同じ人を別の表記で複数回挙げても出席者は1人にまとめる。該当が1人に定まらない名前が1つでもある場合は、依頼を書き出さずに聞き返して終了する(部分的に解決できた分だけで先に進まない)。聞き返しでは、登録が無い名前はその旨を、複数人が該当する名前は該当者のフルネームとメールアドレスの一覧を示す。解決できた参加者は、開催者が打った文字列ではなく名簿のフルネームで以降扱う(確認の提示が入力の写しになっていると、姓だけの指定で別人に解決されても気づけないため)
  3. 依頼IDを作る。起動時刻と乱数から作り、同じ時刻に複数の依頼が立っても重ならない値にする
  4. 依頼の内容(件名・参加者・所要時間・探索条件)をチャットに提示し、開催者の確認を得る。誤りがあればここで作り直す
  5. 依頼を依頼フォルダへ書き出す。アジェンダの原文と絞り込みの条件も依頼に含める(再開時に復元するため)。ファイル名には依頼IDと試行番号を含める。試行番号は初回が1で、代替案2の依頼が2になる(それ以上は増えない)
...
- **手順**:
  ...
  2. 候補ファイルに失敗理由が入っている場合は、候補の絞り込みへ進まずその理由を伝えて終了する。ただし代替案2の候補ファイルの失敗は[候補が0件のときの代替案の提示](#候補が0件のときの代替案の提示)の手順5で扱い、ここでは終了させない
  ...
  7. 採用した枠が0件の場合は[候補が0件のときの代替案の提示](#候補が0件のときの代替案の提示)へ進む
  ...
  12. 選択画面のURLと件名・候補件数を通知として通知フォルダへ書き出し、同じ内容をチャットにも表示する。代替案を提示する場合は、どちらの代替案によるものか、代替案1の枠の仮の予定を持つ参加者と予定の件名、代替案2の広げた後の条件も添える。通知文には、開けない場合は数十秒待って再読込する旨を添える
...
- **対象**: 既定の条件での絞り込みの結果が0件だった依頼
- **手順**:
  1. 2つの代替案を同時に走らせる。開催者への確認は挟まない
  2. **代替案1(仮の予定を含める)**: いま手元にある同じ候補ファイルを、仮の予定を空きとみなす条件で[候補の絞り込みと選択画面の用意](#候補の絞り込みと選択画面の用意)の手順5から絞り直す。期間・時間帯・曜日は変えない。**探索の依頼は出し直さない**(仮かどうかは候補ファイルの空き状況に入っているため)
  3. 代替案1で枠が残った場合は、その枠に仮の予定を持つ参加者の予定の件名を取りに行く。採用した枠を含む範囲と対象の参加者のメールアドレスを予定詳細の依頼として書き出し、予定詳細フローの結果を待つ
  4. **代替案2(期間と時間帯を広げる)**: 期間を当日から3週間、時間帯を9時30分から18時30分に広げた依頼を次の試行番号で書き出し、その候補ファイルの到着を待つ。**開催者が明示的に指定した項目は広げない**。広げられる項目が1つもない場合は代替案2を作らない
  5. 手順3の予定詳細と手順4の候補は同時に待ち、それぞれに待ち上限を適用する。片方だけが得られなかった場合は、得られた側を提示し、得られなかった側はその理由を添えて伝える。**「得られなかった」には、待ち上限を超えた場合・結果ファイルに失敗理由が入っていた場合・依頼を書き出せなかった場合を含める**。代替案2の候補ファイルに失敗理由が入っていた場合も、[候補の絞り込みと選択画面の用意](#候補の絞り込みと選択画面の用意)の手順2の一般則(失敗理由があれば終了する)には従わず、代替案1の枠の提示を続ける(手元にある枠を出せないまま終わらせない)
  6. 代替案の依頼は既定の依頼から間を置かずに続くため、フォルダ監視トリガーの検知間隔が絞られて往復が伸び、待ち上限を超えて打ち切られることがある。これを異常として扱わず、打ち切りとして伝えて依頼IDでの再開に委ねる
  7. 2つの代替案の枠を1枚の選択画面にまとめ、どちらの代替案によるものか、代替案1の枠には仮の予定を持つ参加者と予定の件名、代替案2の枠には広げた後の条件を添えて示す。件名が取得できなかった参加者と、非公開に設定された予定は件名を示さずその旨を書く
  8. 代替案1が0件で、かつ受け取った枠の件数が依頼で要求した最大件数と同数だった場合は、応答が件数で打ち切られている可能性も併せて伝える
  9. 両方の代替案とも0件の場合は、候補が見つからないことと試した条件(代替案2を作らなかった場合は既定と代替案1の2通り、作った場合は3通り)、広げなかった項目(開催者が指定したため広げなかったこと)を挙げて、参加者・期間・所要時間のどれを緩めるかを開催者に相談して終了する
  10. 代替案の候補からさらに自動で条件を緩めない
  11. どこまで進んだかを別に記録しない。代替案を提示したかどうかは、予定詳細の依頼または試行番号2の依頼があるかで判定する([データ設計台帳ファイル](#データ設計台帳ファイル))
  12. 代替案の台帳がある依頼を依頼IDを指定して再開した場合は、通常の絞り込みへ進まず「代替案の提示済み」として手順7から入り、代替案1と代替案2の両方を絞り直して1枚の選択画面にまとめ直す([状態管理](#状態管理))。試行番号2の候補だけを絞り込んで代替案1の枠を落とさない
- **関連するビジネスルール**: [requirements.md#候補が0件のときの代替案の提示](requirements.md#候補が0件のときの代替案の提示)、[requirements.md#候補の提示](requirements.md#候補の提示)
...
- **手順**:
  ...
  2. 読み取った依頼IDと試行番号の候補ファイルを開き、貼られた開始時刻・終了時刻と一致する枠がその中に1つ存在することを確認する。候補ファイルの枠は世界標準時で届き、貼られた時刻は日本時間なので、**文字列ではなく時点として比較する**(候補ファイルの枠を日本時間に変換してから突き合わせる)。存在しない場合は書き出さずに聞き返す(古い画面や別の依頼の内容を貼った場合に取り違えないため)。候補番号は選択画面上の並び順を表す表示用の番号なので突き合わせには使わない。これにより、絞り込みを再現しなくても貼られた内容を検証できる
...
- **手順**:
  ...
  2. 作成の途中で失敗した場合は会議が実際には作られていることがあるため、Teamsのカレンダーでその枠に会議が無いことを確かめてから再試行するよう案内する
  3. 開催者が再試行を指示した場合に限り、再試行番号を1つ増やした選択結果を書き出す。内容は前回と同じにする。ファイル名に再試行番号を含めるため、作成フローのトリガー(新しいファイルの作成)が改めて発火する
  4. 再試行番号を含む作成結果の到着を待ち、以降は[選択結果の受け取りと会議の作成](#選択結果の受け取りと会議の作成)の手順8以降と同じに進む
  5. 再試行は自動では行わない。何度失敗しても、そのたびに開催者の指示を待つ
- **関連するビジネスルール**: [requirements.md#会議の作成](requirements.md#会議の作成)、[requirements.md#候補の選択](requirements.md#候補の選択)

### 往復の待ち合わせと打ち切り

- **対象**: 候補ファイルの到着待ち、予定詳細ファイルの到着待ち、作成結果ファイルの到着待ち
- **手順**:
  ...
  4. その待ちの上限(候補の待ち・予定詳細の待ち・作成結果の待ちで別々に持ち、いずれも既定5分)を超えたら待ちを打ち切る。打ち切ったことと、依頼IDを指定して再度起動すれば続きから再開できること、フロー側の異常のほかにOneDrive同期が止まっている可能性も切り分け先になることを伝えて終了する
...
- **参加者**: 名簿で一人に定まること。定まらない名前が1つでもあれば依頼を書き出さず、複数人が該当する場合はその一覧を示す
...
- **フローは失敗しても結果ファイルを書く。** 探索フロー・作成フロー・予定詳細フローは、コネクタの呼び出しが失敗した場合も失敗理由を入れた結果ファイルを書き出す。Skillは結果ファイルに失敗理由があればその時点で理由を伝えて終了する(待ち上限まで待たされない)。ただし代替案の2つの結果は独立して扱い、片方の失敗でもう片方の提示を止めない
- **同期フォルダへの書き込み失敗はそこで止める。** 依頼・選択結果・通知・HTMLのいずれも、書き出しに失敗したら次の手順へ進まない。ただし代替案の依頼(予定詳細の依頼・代替案2の依頼)の書き出し失敗は例外とし、書き出せた側の代替案だけを提示して、書き出せなかった側はその旨を伝える(2つの代替案は独立しており、片方が作れなくても選択に進める)。通知の書き出し失敗も例外とし、チャットへの表示を行って処理を続ける(通知はOneDrive同期とフローに依存するため、そこが止まっても機能全体を使えなくしない)
...
- `apps/meeting-setup-automation/application/request.py` — 依頼IDの生成、依頼の組み立てと書き出し、候補が0件のときの代替案の組み立て
- `apps/meeting-setup-automation/application/detail.py` — 予定詳細の依頼の書き出しと、予定詳細から枠に重なる仮の予定の件名を取り出す処理
...
| 候補の待ち・絞り込み・代替案の提示・予定詳細の取得・選択画面の書き出し・同期の猶予待ち・ビューアURLの組み立て・通知の書き出し。および選択済みの依頼を再開したときの作成結果の待ちと完了の通知、作成失敗を検知したときの失敗理由の提示 | `main.py resume` |
...
| 予定詳細の依頼 | `auto/meetingSetting/detailRequest/` | Skill | 予定詳細フロー | `detail-request-<依頼ID>.json` |
| 予定詳細 | `auto/meetingSetting/detail/` | 予定詳細フロー | Skill | `detail-<依頼ID>.json` |
...
- 依頼IDは「起動時刻(年月日と時分秒)+乱数4文字」の形にする。試行番号は初回が1で、代替案2の依頼が2になる(それ以上は増えない)。再試行番号は初回の作成が0で、会議の作成をやり直すたびに1増える
- **フローが書き出すファイルの名前は、読んだファイルの名前の先頭を置き換えて決める。** 探索フローは `request-` を `candidates-` に、作成フローは `selection-` を `result-` に、予定詳細フローは `detail-request-` を `detail-` に置き換える。フローは番号を解釈せず、名前を組み立てる判断はSkill側だけが持つ(設計判断2)
- 選択結果のファイル名を決めるのはSkillで、再試行番号が0のときは接尾辞を付けず、1以上のときだけ `-r<再試行番号>` を付ける
- 日時はISO 8601形式で日本時間のオフセット付きで書く
- **通知ファイルの中身はHTML断片で書く。** 会議設定通知フローは中身をそのままTeamsの投稿本文に渡し、Teamsは本文をHTMLとして描画する。改行は`<br>`、URLは`<a href="…">`にし、件名・参加者名など通知に載せる値はエスケープする(素のテキストのままだと全行が1行に潰れ、URLがクリックできず、山括弧を含む値がタグとみなされて消える)。チャットに出す文は同じ内容をプレーンテキストのまま出す
- 作成結果まで到達した依頼のファイルは削除しない。ただし予定詳細ファイルは、他人の予定の件名を残さないため選択が済んだ時点で削除する([セキュリティ](#セキュリティ))
...
「開催者が指定した項目」を持つのは、候補が0件のときの自動の緩和を開催者の指定に作用させないため([候補が0件のときの代替案の提示](#候補が0件のときの代替案の提示))。既定値と同じ値を開催者が明示的に指定した場合も指定として扱うため、値の比較では区別できない。

どこまで進んだかは別に持たない。判断は台帳ファイルの有無と試行番号から導く。

- **代替案をすでに提示したか** — 予定詳細の依頼ファイル、または試行番号2の依頼ファイルがあるかどうか
- **代替案2を作れるか** — 開催者が指定した項目に探索期間と時間帯のどちらかが含まれていないこと。両方が指定されていれば代替案2は作らない
- **ある枠が代替案1のものか代替案2のものか** — その枠が試行番号1の候補ファイルにあるか、試行番号2の候補ファイルにあるか

代替案1は依頼を出し直さないため、試行番号1の候補ファイルを絞り直した結果になる。同じ候補ファイルを同じ条件で絞り直すので、打ち切った後に別のセッションで再開しても同じ枠・同じ表示になる。
...
- **依頼の識別** — 依頼ID
- **取得の指示(フローが読む)** — 対象の参加者のメールアドレス、取得する範囲の開始と終了

### 予定詳細ファイルの区分

- **依頼の識別** — 依頼ID
- **参加者ごとの予定** — 参加者ごとに、予定の状態(空き・仮・予約済み等)・件名・非公開かどうか・開始・終了
- **失敗理由** — フローの処理が失敗した場合にその内容(正常時は空)

予定の開始・終了は世界標準時で届くため、Skillが日本時間に変換してから枠と突き合わせる。件名が取得できないのは次の場合で、いずれも「取得できない」として扱う。

- その参加者のカレンダーが開催者のカレンダー一覧に無い(共有されていない、または追加されていない)
- 予定が繰り返しで、返ってくるのが繰り返しの親だけになる(回ごとに展開されない)
- 予定が非公開に設定されている(件名を示さない)
...
| 予定詳細フロー | 予定詳細の依頼フォルダにファイルが作成されたとき | 「カレンダーの取得 (V2)」でカレンダーの一覧を取り、**持ち主のメールアドレスが指示された参加者と一致するカレンダー**だけを対象に「イベントの取得 (V4)」を呼んで、参加者ごとの予定と失敗理由を予定詳細ファイルとして書き出す。一致するカレンダーが無い参加者は「件名を取得できない」として書き出す。予定詳細のファイル名は、読んだ依頼のファイル名の `detail-request-` を `detail-` に置き換えて決める |
...
`POST /me/events`だけをOffice 365 Outlookコネクタのパススルー経由で呼ぶ。会議オプション画面の直リンクは応答の会議本文から取り出す必要があり、本文を返す経路を選ぶ。

**予定の取得にパススルーは使わない。** このコネクタのパススルーで通るのは`POST /me/events`だけで、`me/calendar/getSchedule`のような3階層のパスやクエリ付きのGETはコネクタの検証で弾かれる([architecture.md#技術的制約](../architecture.md#技術的制約))。代わりに専用アクションの「カレンダーの取得 (V2)」と「イベントの取得 (V4)」を使う。後者は`Calendar id`でカレンダーを指定する形だが、前者が返す一覧に**カレンダーの持ち主のメールアドレス**が含まれるため、参加者のメールアドレスから対象のカレンダーを引ける。**「イベントの取得 (V4)」が返す主催者の欄には接続アカウントが入り、実際の主催者ではないため使わない。**
...
- 代替案として提示する場合は、代替案1の枠と代替案2の枠を見出しで分けて並べる。代替案1の枠には仮の予定を持つ参加者の名前と予定の件名を、代替案2の枠には広げた後の期間と時間帯を添える。件名を取得できなかった参加者と非公開に設定された予定は、件名の代わりに取得できないことを示す
- 参加者の空き状況は「全員空き」または空いていない人数だけを示す。予定の件名を示すのは、代替案1の枠で仮の予定を持つ参加者についてだけとする
...
クリップボードへ入れる1行は、目印の語・依頼ID・試行番号・候補番号・開始時刻・終了時刻を並べたものにする。**試行番号は画面全体で1つではなく、その枠が入っていた候補ファイルの試行番号をカードごとに埋め込む**(代替案1の枠は1、代替案2の枠は2)。1枚の画面に2つの試行番号の枠が並ぶため、画面単位で最大の試行番号を使うと代替案1の枠が突き合わせで弾かれる。時刻は日本時間のオフセット付きで書く(台帳の日時と同じ書き方に揃え、貼られた側が時点として解釈できるようにする)。目印の語を先頭に置くことで、貼られた内容が選択結果かどうかを取り違えずに判定できる。
...
**表は上から順に当てはめ、最初に一致した行をその依頼の状態とする。** 後の工程の状態を先に置いてあるため、先の工程の行が後の工程を食い潰すことがない。

| 状態 | 判定 | 次にすること |
|---|---|---|
| 作成済み | 最後の再試行番号の作成結果があり、失敗理由が入っていない | 完了内容を伝える |
| 作成失敗 | 最後の再試行番号の作成結果があり、失敗理由が入っている | 失敗理由を伝え、開催者の指示があれば再試行する |
| 選択済み | 最後の再試行番号の選択結果がある | 作成結果の到着を待つ |
| 代替案の提示済み | 代替案の台帳(予定詳細の依頼・試行番号2の依頼)があり、それらに対応する結果が揃っている | 代替案1と代替案2の両方を絞り直して1枚の選択画面にまとめ、選択を受け取る |
| 代替案の提示中 | 代替案の台帳があり、対応する結果がまだ揃っていない | 予定詳細と代替案2の候補の到着を待つ |
| 候補到着 | 試行番号1の候補がある | 既定の条件で絞り込み・選択画面の用意・選択の受け取り |
| 依頼済み | 試行番号1の依頼がある | 候補の到着を待つ |

再試行番号は、その依頼IDの選択結果ファイルのうち最も大きい番号を今の再試行とする。試行番号は状態の判定には使わず、代替案2の依頼と候補を試行番号2のファイル名で指すためだけに使う(依頼ファイルの最大の試行番号で状態を決めると、代替案2の依頼を書いた直後に「依頼済み」へ戻ってしまう)。

「代替案の提示済み」で結果が揃っているかは、書き出した台帳に対応する結果があるかで見る。予定詳細の依頼だけを書いた場合は予定詳細が、試行番号2の依頼だけを書いた場合はその候補が、両方書いた場合は両方が揃った時点で「提示済み」になる。選択結果があれば代替案の2行には到達しないため、予定詳細ファイルを選択後に削除しても([セキュリティ](#セキュリティ))、来ないファイルを待ち続けることはない。

下の図の「打ち切り」と「相談」は、上の表の状態ではなく、待ちをやめて終了した位置を表す。台帳ファイルから導けるのは表の7状態だけで、打ち切って終了した依頼も、どちらの代替案でも候補が出ずに相談で終わった依頼も、台帳の上では直前の状態のまま残る。相談で終わった依頼を依頼IDを指定して再開した場合は、代替案の台帳があれば「代替案の提示済み」として、台帳が増えていなければ「候補到着」として同じ絞り込みをやり直し、いずれも同じ結論(相談)に至る。
...
stateDiagram-v2
    ...
    依頼済み --> 候補到着: 候補が届いた
    依頼済み --> 打ち切り: 待ち上限を超えた
    候補到着 --> 代替案の提示中: 0件で2つの代替案を調べる
    候補到着 --> 相談: 代替案1が0件で代替案2も作れない
    代替案の提示中 --> 代替案の提示済み: 代替案の結果が揃った
    代替案の提示中 --> 打ち切り: 待ち上限を超えた
    代替案の提示済み --> 相談: どちらの代替案も0件
    代替案の提示済み --> 選択済み: 選択結果を書き出した
    代替案の提示中 --> 選択済み: 届いた側の枠で選択結果を書き出した
    ...
    作成失敗 --> [*]: 再試行しない
    相談 --> 代替案の提示済み: 依頼IDを指定して再開する
    相談 --> 候補到着: 代替案の台帳が無い依頼を再開する
    打ち切り --> 依頼済み: 依頼IDを指定して再開する
    打ち切り --> 代替案の提示中: 依頼IDを指定して再開する
...
- **聞き返しに出す候補一覧はチャットへの表示にとどめる。** 複数人が該当したときに示すフルネームとメールアドレスは標準出力にだけ出し、ログファイルには残さない(ログにメールアドレスを出さない制約を崩さない)。ログには「何件該当したか」だけを残す
...
- **他人の予定の件名を扱うのは、候補が0件のときの代替案1に限る。** 通常の候補では空いているかどうかだけを扱い、探索フローも空き状況以上のものを候補ファイルに書かない。代替案1では仮の予定に重ねてよいかの判断材料として件名が必要なため、予定詳細フローで件名を取得し、予定詳細ファイル・候補選択画面・Teams通知・チャットに載せる。ただし予定の本文と出席者は扱わず、非公開に設定された予定の件名も示さない。置き場所は個人のOneDriveと自分専用のTeamsチャネルの範囲に留める
- **予定の件名は使い終わったら残さない。** 予定詳細ファイルはその依頼の選択が済んだ時点で削除する(台帳の他のファイルは残す)。どの会議をこの機能で設定したかを追うのに他人の予定の件名は要らない
...
- **ログに個人情報と会議内容を出さない。** メールアドレス・アジェンダ本文・参加URL・会議本文・他人の予定の件名はログに出さず、依頼IDと件数・状態だけを残す
...
| 予定詳細の取得 | 依頼ID・対象の参加者の人数・件名を取得できた件数 | INFO |
| どちらの代替案も0件 | 依頼ID・試した条件 | WARNING |
...
| 候補の提示-7 | 単体 | 0件のときの代替案の提示への分岐 |
| 候補の提示-8 | 単体 | どちらの代替案によるものかと、広げた後の条件が選択画面・通知・チャットの文に入ること |
...
| 往復の待ち合わせ-5 | 結合 | 2つの待ちのうち片方だけが打ち切られた場合に、届いた側を提示して届かなかった側を打ち切りとして伝えること |
...
| 依頼内容の受け付け条件-6 | 単体 | アジェンダ未指定でも受け付け、無いことが分かる本文にする |
| 依頼内容の受け付け条件-7 | 単体 | 満たさない場合に依頼を書き出さず聞き返す |
| 依頼内容の受け付け条件-8 | 単体 | 参加者の人数で依頼を弾かないこと |
...
| 候補探索の既定条件-5 | 単体 | 既定では仮の予定を空きとみなさないこと。代替案1ではみなすこと |
| 候補が0件のときの代替案の提示-1 | 単体 | 2つの代替案を両方作ること。代替案1は同じ候補ファイルを絞り直し、探索の依頼を出し直さないこと |
| 候補が0件のときの代替案の提示-2 | 単体 | 代替案1の枠に仮の予定を持つ参加者と件名を添えること。件名が空・非公開の予定は件名を示さないこと |
| 候補が0件のときの代替案の提示-3 | 単体 | カレンダーが見つからない参加者と繰り返し予定を、件名を取得できないものとして扱うこと |
| 候補が0件のときの代替案の提示-4 | 単体 | 開催者が指定した項目を広げないこと。両方指定されていれば代替案2を作らないこと |
| 候補が0件のときの代替案の提示-5 | 単体 | どちらの代替案によるものかが分かる形で示すこと |
| 候補が0件のときの代替案の提示-6 | 単体 | 代替案からさらに緩めず、両方0件なら相談に切り替えること |
| 候補が0件のときの代替案の提示-7 | 単体 | 受け取った枠が要求件数と同数のときに打ち切りの可能性を伝えること |
| 候補が0件のときの代替案の提示-8 | 結合 | 代替案の提示は依頼1件につき1回。台帳の有無から判定でき、再開しても同じ結果になること |
| 往復の待ち時間の上限-1 | 単体 | 候補の待ち・予定詳細の待ち・作成結果の待ちの上限をそれぞれ持ち、いずれも既定が5分であること |
| 往復の待ち時間の上限-2 | 単体 | 待ちごとに上限を適用する |
| 往復の待ち時間の上限-3 | 結合 | 再探索で打ち切られても台帳を残し、依頼IDを指定した再開で続けられること |
| 参加者の解決-1 | 単体 | Git管理外の名簿の読み込み |
| 参加者の解決-2 | 単体 | 登録なし・複数人該当は中断する |
| 参加者の解決-3 | 単体 | 名簿にない相手は参加者にならない |
| 参加者の解決-4 | 単体 | 表記ゆれの吸収と、姓だけ・名だけの指定 |
| 参加者の解決-5 | 単体 | 複数該当時にフルネームとメールアドレスの一覧を示す |
| 参加者の解決-6 | 単体 | 照合の優先順位と、フルネーム該当と姓・名該当の合算 |
| 参加者の解決-7 | 単体 | 同じ人を別表記で複数回指定しても出席者は1人 |
