...
- **対象**: 絞り込みの結果が0件だった依頼
- **手順**:
  1. まだ緩められる項目があるかを調べる。時間帯を緩められるのは「開催者が時間帯を指定しておらず、依頼ファイルの時間帯が既定値と同じで、かつその起動でまだ絞り直していない」とき、期間を緩められるのは「開催者が探索期間を指定しておらず、試行番号が1」のとき。時間帯の絞り直しは1回の起動につき1回だけ行い、2度目はここへ戻ってきても期間の緩和へ進む
  2. 時間帯が開催者の指定でなく、まだ緩めていない場合は、時間帯を9時30分から18時30分に広げた条件で、いま手元にある同じ候補ファイルを[候補の絞り込みと選択画面の用意](#候補の絞り込みと選択画面の用意)の手順5から絞り直す。絞り直したことを覚えておき、候補が出た場合は緩めた後の条件を選択画面と通知に示す。対象とする曜日は変えない。**探索の依頼は出し直さない**(時間帯の絞り込みはこちら側だけで行い、探索の指示に時間帯は含まれないため、出し直しても同じ枠が返るだけで往復が1回増える)
  ...
  8. どこまで緩めたかを別に記録しない。判断は依頼ファイルと試行番号、および試行番号1での絞り直しの有無から導ける([データ設計台帳ファイル](#データ設計台帳ファイル)の依頼ファイルの区分)。打ち切りや相談で終わった後に別のセッションで再開しても同じ判定・同じ表示になる
...
どこまで緩めたかは別に持たない。判断は次の2つに書き分ける。

- **これから緩められるか**(緩和を進めるかの判断) — 時間帯は「開催者が時間帯を指定しておらず、依頼ファイルの時間帯が既定値と同じで、かつその起動でまだ絞り直していない」、期間は「開催者が探索期間を指定しておらず、試行番号が1」。時間帯は起動を単位に、期間は依頼の一生を単位に数える
- **すでに緩めたか**(選択画面と通知に緩めた後の条件を示すかの判断) — 時間帯は「開催者が時間帯を指定しておらず、かつ『依頼ファイルの時間帯が既定値と違う』または『その起動で絞り直した』」、期間は「開催者が探索期間を指定しておらず、試行番号が2以上」

時間帯の緩和は依頼を出し直さないため、試行番号1のあいだは依頼ファイルの時間帯が既定値のままになる。この1回だけは「その起動で絞り直した」で拾う。期間を広げた依頼には緩めた後の時間帯を書き込むため(手順3)、試行番号2以降は依頼ファイルの値から導ける。したがって打ち切った後に別のセッションで再開しても、同じ判定・同じ表示になる。
...
下の図の「打ち切り」と「相談」は、上の表の状態ではなく、待ちをやめて終了した位置を表す。台帳ファイルから導けるのは表の5状態だけで、打ち切って終了した依頼も、緩められる項目がなくなって相談で終わった依頼も、台帳の上では直前の状態のまま残る。相談で終わった依頼を依頼IDを指定して再開した場合は、同じ候補を同じ条件で絞り直して同じ結論(相談)に至る。
...
| 候補が0件のときの探索条件の拡張-5 | 単体 | 時間帯の絞り直しは1回の起動につき1回まで、期間の拡張は依頼1件につき1回まで |
