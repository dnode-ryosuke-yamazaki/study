...
どこまで緩めたかは別に持たない。**これから緩められるか**は、時間帯なら「開催者が時間帯を指定しておらず、依頼ファイルの時間帯が既定値と同じ」、期間なら「開催者が探索期間を指定しておらず、試行番号が1」から導ける。**実際に緩めたか**(選択画面と通知に緩めた後の条件を示すかの判断)は、その実行の中で絞り直しを行ったかどうかで決まる。時間帯の緩和は依頼を出し直さないため、依頼ファイルの時間帯は緩めた後も既定値のままである点に注意する。打ち切った後に再開した場合も、依頼ファイルの条件から同じ順で絞り直すため同じ結果・同じ表示になる。
