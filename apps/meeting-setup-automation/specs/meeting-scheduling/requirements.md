...
- [3] 候補は参加者全員が空いている枠のみとすること。ただし依頼時に一部不在の枠も含めるよう指示された場合はその指示に従うこと。根拠: 全員が空いている枠を提示するのが既定の期待であり、例外は依頼のたびに開催者が判断する
...
- [1] 候補の待ちと作成結果の待ちのそれぞれについて待ち時間の上限を設け、いずれも5分とすること。根拠: どちらの待ちも「台帳ファイルの書き出し → フォルダ監視トリガーの検知 → フロー内のアクション実行 → 結果ファイルのOneDrive同期」という同じ経路をたどるため、同じ決め方をする。上限は、その待ちの往復時間の3倍を下回らない値のうち切りのよい分数で最小のものとする。3倍にするのは、同期の遅れが平常時の倍以上に振れても打ち切らないため。往復時間が100秒以内であれば3倍は5分以内に収まるため、5分を上限の下限値として用いる【推測】
- [2] 往復時間は候補の待ちと作成結果の待ちのそれぞれについて実測し([tasks.md](tasks.md#仕様承認pr前に行う実機確認)のE1とE2)、3倍が5分を超えていた場合(往復時間が100秒を超えていた場合)は、その待ちの上限を「実測の3倍を分単位に切り上げた値」に置き換えること。候補の待ちと作成結果の待ちで上限が別の値になってよい
