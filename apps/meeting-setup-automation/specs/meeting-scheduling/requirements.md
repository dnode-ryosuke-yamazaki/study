...
- [5] 同一の依頼IDについて選択結果を書き出すのは1回までとすること。ただし会議の作成が失敗した場合は、[会議の作成](#会議の作成) [5] の再試行として、前回と区別できる形でもう一度書き出せること【推測】
...
- [5] 会議の作成が失敗した場合は、失敗の理由を伝えたうえで、開催者が同じ枠での再試行を指示したときに限り作成をやり直せること。再試行を自動で行わないこと【推測】
...
- [3] 時間帯は開始時刻が終了時刻より前で、その差が所要時間以上あること。根拠: 差が所要時間未満だと候補が原理的に0件になる
- [4] 提示する候補件数は1件以上であること。根拠: 0件の指定は候補を提示しない依頼になり、依頼として意味を持たない
- [5] [1]から[4]のいずれかを満たさない場合は依頼を書き出さず、満たしていない項目を示して聞き返すこと
...
- [1] 候補の待ちと作成結果の待ちのそれぞれについて、待ち時間の上限を5分とすること。根拠: 依頼の書き出しから候補がローカルで読めるまでの実測時間(フォルダ監視トリガーの検知間隔・OneDrive同期のラグ・フロー内のアクション実行の合計)に対して余裕を持たせた値で、これを超える場合はフロー側の異常を疑うべき状態にあたる。実測値は[tasks.md](tasks.md#仕様承認pr前に行う実機確認)のE1で確認し、実測が5分に迫る場合はこの値を実測値に合わせて見直す【推測】
...
- [1] 録画の自動開始とファシリテーター機能の有効化は自動化せず、会議オプション画面の直リンクから開催者が手動で有効化すること。根拠: 会議オプション相当の設定はオンライン会議のリソースにしか存在しない。このリソースへ到達できるのはMicrosoft Teamsコネクタのパススルーだけだが、その接続にはオンライン会議を読み書きする権限が無く、接続を作り直しても権限エラーで止まる。またファシリテーター機能には有効化を指示するAPIのプロパティ自体が存在せず、会議ごとに画面上で有効化する仕様である
