...
- [5] 2つの待ちを同時に行っていて片方だけが得られなかった場合は、全体を終了せず、得られた側の結果を提示したうえで、得られなかった側の理由を伝えること。得られなかった場合には、待ち上限を超えた場合・結果に失敗理由が返った場合・依頼を書き出せなかった場合を含めること。根拠: 候補が0件のときの2つの代替案は独立しており、片方が得られれば選択に進める
...
- [8] 代替案の提示は依頼1件につき1回とすること。すでに提示したかどうかは台帳ファイルの有無から判定できること。打ち切りのあとに同じ依頼IDで再開した場合は同じ2つの代替案を作り直すが、同じ台帳から同じ条件で作るため結果は変わらないこと。代替案の台帳が増えないことがあり、その場合も再開したときは既定の絞り込みからやり直して同じ結果に至ること。台帳が増えないのは、代替案1が0件で代替案2も作れない場合
