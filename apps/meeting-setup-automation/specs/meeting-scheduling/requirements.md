...
- [1] 候補の待ちと作成結果の待ちのそれぞれについて、待ち時間の上限を5分とすること。根拠: 依頼の書き出しから候補がローカルで読めるまでの往復時間(フォルダ監視トリガーの検知間隔・OneDrive同期のラグ・フロー内のアクション実行の合計)の3倍を下回らない値のうち、切りのよい分数で最小のものを上限とする。3倍にするのは、同期の遅れが平常時の倍以上に振れても打ち切らないため。往復時間が100秒以内であれば3倍は5分以内に収まるため、5分を上限の下限値として用いる【推測】
