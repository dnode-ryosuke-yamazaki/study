...
- [8] 自動で条件を緩めて候補が出た場合は、緩めた後の条件を選択画面と通知に示すこと。根拠: 既定の時間帯の外にある枠が候補に出た理由が伝わらないと、条件を取り違えて提示されたと誤解される【推測】
...
- [5] 空いているかどうかの判定では、仮の予定が入っている状態を空いていないものとして扱うこと。根拠: 仮でも予定が入っている枠を候補に出すと、選ばれた後に先約との調整が必要になり日程調整をやり直すことになる【推測】

### 候補が0件のときの探索条件の拡張

- [1] 開催者が時間帯を指定しておらず候補が0件の場合、時間帯を9時30分から18時30分に広げた条件で、受け取った候補をそのまま絞り直すこと。対象とする曜日は変えないこと。**このとき探索の依頼は出し直さないこと。** 絞り直しは1回の起動につき1回だけ行うこと。打ち切りや相談のあとに同じ依頼IDで再開した場合は改めて絞り直すが、同じ候補を同じ条件で絞り直すため結果は変わらないこと。 根拠: 時間帯の絞り込みはこちら側だけで行っており、探索の依頼に時間帯は含まれないため、依頼を出し直しても同じ枠が返るだけで往復が1回増える【推測】
- [2] それでも0件で、かつ開催者が探索期間を指定していない場合、期間を当日から3週間に広げて再探索すること(時間帯と対象とする曜日は前段の値のまま)。[1]の緩和が開催者の指定でスキップされた場合も適用すること
- [3] 緩められる項目が残っていない場合は、さらに自動で緩めず、候補が見つからないことと試した条件を報告し、参加者・期間・所要時間のどれを緩めるかを開催者に相談すること。根拠: これ以上の拡張は「本来なら避けたい枠」に踏み込む判断であり、自動で決めるべきではない
- [4] 自動での緩和は開催者が指定していない項目にだけ作用させること。根拠: 開催者が明示した条件を自動で緩めるのは、避けたい枠に踏み込む判断を代わりに行うことにあたり、[3]と同じ理由で自動で決めるべきではない【推測】
- [5] 自動での緩和は、時間帯の絞り直し([1])を1回の起動につき1回まで、期間の拡張([2])を依頼1件につき1回までとすること。根拠: 時間帯の絞り直しは受け取った候補を絞り直すだけで結果が起動のたびに変わらないため起動を単位にでき、期間の拡張は依頼を1件増やすため依頼の一生を単位にする必要がある
- [6] 時間帯を緩めても候補が0件で、受け取った枠が探索で要求した最大件数と同数だった場合は、応答が件数で打ち切られている可能性を併せて伝えること。根拠: 空き時間の応答は要求した件数で打ち切られ、打ち切られた集合に緩めた時間帯の枠が入っていないことがある。探索の依頼を出し直しても同じ枠が返るため、緩和では回復できない【推測】
...
- [3] 前の依頼から間を置かずに続く再探索では上限を超えて打ち切られることがあり、それを異常として扱わないこと。打ち切ったあとも台帳を残し、依頼IDを指定した再開で続けられること。根拠: フォルダ監視トリガーの検知間隔は短時間に連続して依頼を出すと絞られ、7分間隔での連続実行では往復が670秒まで伸びる。候補が0件のときの期間の拡張は間を置かずに次の依頼を出すため、これに当たる
