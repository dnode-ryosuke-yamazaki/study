...
- [2] 代替案1の候補には、仮の予定が入っている参加者と、その予定の件名を添えて、選択画面・Teams通知・チャットのそれぞれに示すこと。件名を取得できない参加者については取得できないことを示し、非公開に設定された予定の件名は示さないこと。予定の本文と出席者は扱わないこと。根拠: 仮の予定に重ねてよいかは予定の内容で判断が変わるため、判断材料として示す
- [3] 件名を取得できるのは、その参加者が開催者にカレンダーを共有しており、かつ開催者のカレンダー一覧にそのカレンダーが入っている場合に限ること。それ以外の参加者と、繰り返し予定の回については件名を取得できないものとして扱うこと。根拠: 予定を読める手段が「開催者のメールボックスにマウントされたカレンダー」を対象にしたものしかなく、繰り返し予定はその手段では回ごとに展開されない。共有の有無は参加者側の設定であり、こちらでは変えられない
...
- [8] 代替案の提示は依頼1件につき1回とすること。すでに提示したかどうかは台帳ファイルの有無から判定できること。打ち切りのあとに同じ依頼IDで再開した場合は同じ2つの代替案を作り直すが、同じ台帳から同じ条件で作るため結果は変わらないこと。代替案の台帳が増えないことがあり、その場合も再開したときは既定の絞り込みからやり直して同じ結果に至ること。台帳が増えないのは、代替案1が0件で代替案2も作れない場合と、代替案1に枠が残ったが件名を取りに行く対象が1人もおらず代替案2も作れない場合
