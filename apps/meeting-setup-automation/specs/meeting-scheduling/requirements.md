...
- [1] 開催者が時間帯を指定しておらず候補が0件の場合、時間帯を9時30分から18時30分に広げた条件で、受け取った候補をそのまま絞り直すこと。対象とする曜日は変えないこと。**このとき探索の依頼は出し直さないこと。** 絞り直しは1つの試行につき1回だけ行うこと。 根拠: 時間帯の絞り込みはこちら側だけで行っており、探索の依頼に時間帯は含まれないため、依頼を出し直しても同じ枠が返るだけで往復が1回増える【推測】
...
- [5] 自動での緩和は依頼1件につき2回まで(上記[1]と[2])とすること
