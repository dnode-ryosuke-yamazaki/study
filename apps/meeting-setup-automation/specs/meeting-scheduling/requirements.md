...
- [1] 開催者が時間帯を指定しておらず候補が0件の場合、時間帯を9時30分から18時30分に広げて再探索すること。対象とする曜日は変えないこと
- [2] 候補が0件で、かつ開催者が探索期間を指定していない場合、期間を当日から3週間に広げて再探索すること(時間帯と対象とする曜日は前段の値のまま)。[1]の拡張が開催者の指定でスキップされた場合も適用すること
- [3] 広げられる項目が残っていない場合は、さらに自動で拡張せず、候補が見つからないことと試した条件を報告し、参加者・期間・所要時間のどれを緩めるかを開催者に相談すること。根拠: これ以上の拡張は「本来なら避けたい枠」に踏み込む判断であり、自動で決めるべきではない
- [4] 自動での拡張は開催者が指定していない項目にだけ作用させること。根拠: 開催者が明示した条件を自動で緩めるのは、避けたい枠に踏み込む判断を代わりに行うことにあたり、[3]と同じ理由で自動で決めるべきではない【推測】
- [5] 自動での拡張は依頼1件につき2回まで(上記[1]と[2])とすること

### 往復の待ち時間の上限

- [1] 候補の待ちと作成結果の待ちのそれぞれについて待ち時間の上限を設け、いずれも5分とすること。根拠: どちらの待ちも「台帳ファイルの書き出し → フォルダ監視トリガーの検知 → フロー内のアクション実行 → 結果ファイルのOneDrive同期」という同じ経路をたどるため、同じ決め方をする。間隔を空けた単発の往復時間は候補が45〜50秒、作成結果が70秒であり、その3倍を下回らない切りのよい分数が5分にあたる。3倍にするのは、同期の遅れが平常時の倍以上に振れても打ち切らないため
- [2] 拡張による再探索はそれぞれ新しい待ちとして扱い、待ちごとに上限を適用すること
- [3] 前の依頼から間を置かずに続く再探索では上限を超えて打ち切られることがあり、それを異常として扱わないこと。打ち切ったあとも台帳を残し、依頼IDを指定した再開で続けられること。根拠: フォルダ監視トリガーの検知間隔は短時間に連続して依頼を出すと絞られ、7分間隔での連続実行では往復が670秒まで伸びる。候補が0件のときの拡張は間を置かずに次の依頼を出すため、2回目以降がこれに当たる
