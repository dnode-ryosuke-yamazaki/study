...
**チャネル会議は1サイトにつきフロー①が1本必要**です(トリガーが1つのサイトしか監視できないため)。チームを追加するときは、改造済みのフロー①をコピーして監視先を差し替えます。

**バッチとフロー②は変更不要です。** 台帳を集める場所は同じなので、バッチは1つで足ります。

### 手順

1. 既存の `トランスクリプト取得-チーム全体版-台帳版v2` を**エクスポート**する
2. 監視先を差し替える(`--監視先を変える`)
3. zipに固め直して**新規作成としてインポート**する
4. 新しいフローをオンにする

```
cd /Users/ryosyamazaki/repo/study/apps/teams-transcript-fetcher/application/power-automate
python3 patch_flow_definition.py "<エクスポートしたdefinition.json>" "<出力先>/definition.json" --監視先を変える --サイトurl "https://<テナント>.sharepoint.com/sites/<新しいチーム>" --ライブラリ "<ドキュメントライブラリのID>" --フロー名 "トランスクリプト取得-<新しいチーム>"
```

**ライブラリのID**は、既存フローのトリガーの `table` と同じ形式(GUID)です。新しいサイトのものは、Power Automate の編集画面でトリガーのライブラリを選び直したときに入る値を見るか、いったんインポート後にUIで選び直しても構いません。

### なぜスクリプトを使うのか

サイト固有の値が**3箇所**に散っています。

| 箇所 | 忘れるとどうなるか |
|---|---|
| トリガーの監視先 | そもそも起動しない(すぐ気づく) |
| 一覧取得の問い合わせ先 | 一覧取得が失敗する(すぐ気づく) |
| **台帳の `siteUrl`** | **台帳はできてフェーズ1では取得できるが、URL再発行だけ失敗する** |

3つ目が厄介です。**フェーズ1では露見しない**ため、しばらく正常に見えて、PCを止めた後の回復だけが効かなくなります。スクリプトなら3箇所を一度に差し替えるので、この取り違えが起きません。

> 実際にこの取り違えを一度やっています(台帳の `siteUrl` に台帳の保存先を入れていた)。テストで固定してあります。

### 確認

新しいチームのチャネルで会議を録画し、`ledger/` に台帳ができたら中身を見てください。

```
cat ~/Library/CloudStorage/OneDrive-Deloitte\(O365D\)/00_root/auto/transcript/ledger/*.json
```

**`siteUrl` が新しいチームのサイトになっていること**を確認してください。ここが個人OneDriveや元のチームのURLになっていたら差し替え漏れです。
...
## ハートビートフロー(同期停滞の監視・フロー③)

5分間隔で現在時刻を `transcript/_heartbeat.txt` へ上書きするフローです。**バッチはこのファイルの鮮度でOneDrive同期の停滞を検知します**(仕様: [specs/sync-stall-recovery/](../../specs/sync-stall-recovery/))。フロー①②と違いトランスクリプトには触れないため、新規に手で作れる規模です。

### 手順

1. 「作成」→「スケジュール済みクラウド フロー」を選び、名前を `トランスクリプト監視-ハートビート` にする
2. トリガー「Recurrence(繰り返し)」の間隔を **5分** にする
3. アクション「ファイルの作成(SharePoint または OneDrive for Business コネクタ。フロー①と同じ接続)」を追加する
   - フォルダー: `00_root/auto/transcript`
   - ファイル名: `_heartbeat.txt`
   - ファイル コンテンツ: 式 `utcNow()`
4. 「ファイルの作成」は同名ファイルがあると失敗するため、**上書きを許可する**(コネクタに上書きオプションがある場合はオン。無い場合は「ファイルの更新」系アクションとの組み合わせ、または失敗時に更新へフォールバックする並列分岐にする)

**書き込みは毎回同じファイルへの上書きで、ファイル数は増えません**(仕様: requirements.md#ハートビートの書き込み [3])。1回の書き込み失敗はそのまま次回に任せて構いません。停滞判定の閾値(20分)はハートビート4回分に相当し、単発の失敗では停滞と判定されません。

### 反映の順序(既に15分間隔で動いているフローを切り替える場合)

**このフローを5分間隔にするより先にバッチ側のしきい値を配ってはいけません。** 順序を逆にすると、正常なOneDriveを停滞と誤判定して再起動し、Teamsへ停滞の通知が飛びます。

ハートビートが15分間隔のままだと、次の書き込み直前の鮮度は「15分 + 同期ラグ(0〜5分)」で最大20分程度になり、20分のしきい値を超えます。バッチは5分間隔で判定するため、この状態は毎サイクル起こりえます。

**バッチはメインのクローン(`~/repo/study`)の作業ツリーを直接読んで動きます**(登録済みplistの `ProgramArguments` がそのパスを指す)。**GitHubでマージしただけでは本番は変わらず、ローカルを更新した時点で新しいしきい値が効きます。** 手順は次の順で行ってください。

1. このフローの Recurrence を15分から5分に変更する
2. `fetch.log` の `停滞判定: 正常(鮮度=N分)` で N が常に20を下回るようになるのを数サイクル確認する(15分周期のノコギリ波が消え、上限が10分前後に収まる)
3. 実装をmainへマージし、`~/repo/study` が `main` を指していることを確認してから `git pull` で作業ツリーを更新する(この時点でしきい値が20分・20分・45分に切り替わる)。このリポジトリはworktree運用のため、`git worktree list` で対象がメインのクローンであることを確かめる
4. 両アプリのplistを再登録して `WatchPaths` を有効にする

しきい値は手順3の作業ツリーの更新だけで効きますが、`WatchPaths` は再登録しない限り効きません。この非対称があるため、手順3と4は別々に必要です。

逆順で配ってしまった場合は、フローを5分に切り替えれば次のサイクルから収まります。誤って実行された再起動は24時間で2回の上限を消費するため、その日は本物の停滞に対する自動復旧が効かなくなる点に注意してください。

### 動作確認

1. フローを手動実行(またはオンにして5分待つ)する
2. ローカルの同期フォルダに `transcript/_heartbeat.txt` が現れ、中身がUTCのISO 8601日時1行であることを確認する

```
cat ~/Library/CloudStorage/OneDrive-Deloitte\(O365D\)/00_root/auto/transcript/_heartbeat.txt
```

3. バッチのログ(`fetch.log`)に毎サイクル `停滞判定: 正常(鮮度=N分)` が出れば、検知まで通っています

### うまくいかないときに見るところ

**バッチのログに `ハートビートが存在しない` が出続ける**

フローが動いていないか、書き込み先のフォルダが違います。フローの実行履歴と、アクションのフォルダー指定を確認してください。ハートビートが読めない間、バッチは同期停滞の判定を行いません(誤って正常なOneDriveを再起動しないための仕様です)。

**バッチのログに `ハートビートが解釈不能` が出る / ファイルの中身がそのまま文字列 `utcNow()` になっている**

「ファイル コンテンツ」欄に `utcNow()` を**プレーンテキストとして**入力すると、評価されずに文字列そのものが書き込まれます。入力欄を一度クリアし、`fx`(式の追加)から `utcNow()` を式として挿入し直してください。
