...
15分間隔で現在時刻を `transcript/_heartbeat.txt` へ上書きするフローです。**バッチはこのファイルの鮮度でOneDrive同期の停滞を検知します**(仕様: [specs/sync-stall-recovery/](../../specs/sync-stall-recovery/))。フロー①②と違いトランスクリプトには触れないため、新規に手で作れる規模です。

### 手順

1. 「作成」→「スケジュール済みクラウド フロー」を選び、名前を `トランスクリプト監視-ハートビート` にする
2. トリガー「Recurrence(繰り返し)」の間隔を **15分** にする
...
**書き込みは毎回同じファイルへの上書きで、ファイル数は増えません**(仕様: requirements.md#ハートビートの書き込み [3])。1回の書き込み失敗はそのまま次回に任せて構いません。停滞判定の閾値(45分)はハートビート3回分に相当し、単発の失敗では停滞と判定されません。

### 動作確認

1. フローを手動実行(またはオンにして15分待つ)する
