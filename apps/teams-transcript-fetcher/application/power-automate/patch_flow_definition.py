...
def _台帳のアクションを作る(
    録画のサイト: str, 由来: str, ドライブ識別子: str, 録画の識別子: str
) -> dict:
    """台帳の内容を組み立てる。
    ...
    **`siteUrl` は「録画があるサイト」であり、台帳の保存先ではない。**
    フロー②がこの値を使って録画のあるサイトのAPIを呼ぶ。台帳の保存先を
    入れると、チャネル会議のURL再発行が失敗する(他サイトの識別子を
    自分のサイトに問い合わせる形になる)。フェーズ1では露見しないため
    実機確認でも気づけなかった(2026-08-19)。
...
def 書き換える(
    ...
    *,
    ...
    # 録画があるサイトは、一覧取得が実際に問い合わせる先と同じでなければならない。
    録画のサイト = 入れ物[アクション_一覧取得]["inputs"]["parameters"]["dataset"]
    入れ物[アクション_台帳] = _台帳のアクションを作る(
        録画のサイト, 由来, ドライブ識別子, 録画の識別子
    )
...
def 監視先を差し替える(
    定義: dict, *, サイトurl: str, ライブラリ: str, フロー名: str
) -> tuple[dict, list[str]]:
    """既に改造済みのフロー①を、別のTeamsチーム(別サイト)向けに作り替える。

    **チームを追加するときに使う。** サイト固有の値は3箇所に散っているため、
    手で直すと1箇所忘れて「台帳はできるがURL再発行だけ失敗する」といった
    分かりにくい壊れ方をする。ここで一度にまとめて差し替える。

    差し替える3箇所:
    1. トリガーの監視先(どのサイトの録画を見るか)
    2. 一覧取得の問い合わせ先(録画があるサイトのAPIを呼ぶ)
    3. 台帳の `siteUrl`(フロー②が再発行に使う。2と必ず同じ値)

    **台帳の保存先は変えない。** どのチームの録画でも台帳は同じ場所に集める。
    """
    新しい定義 = copy.deepcopy(定義)
    新しい定義["properties"]["displayName"] = フロー名
    入れ物 = _アクションの入れ物を探す(新しい定義)

    足りないもの = [
        名前
        for 名前 in (アクション_一覧取得, アクション_台帳, アクション_ファイルの作成)
        if 名前 not in 入れ物
    ]
    if 足りないもの:
        raise SystemExit(
            "改造済みのフロー①ではないようです(見つからないアクション: "
            + ", ".join(足りないもの)
            + ")。改造前の定義を渡していないか確認してください。"
        )

    トリガー = next(iter(新しい定義["properties"]["definition"]["triggers"].values()))
    トリガー["inputs"]["parameters"]["dataset"] = サイトurl
    トリガー["inputs"]["parameters"]["table"] = ライブラリ

    入れ物[アクション_一覧取得]["inputs"]["parameters"]["dataset"] = サイトurl
    入れ物[アクション_台帳]["inputs"]["siteUrl"] = サイトurl

    変更点 = [
        f"トリガーの監視先を変更: {サイトurl}",
        f"監視するライブラリを変更: {ライブラリ}",
        "一覧取得の問い合わせ先と台帳のsiteUrlを監視先に合わせた",
        "台帳の保存先は変更していない(すべてのチームで同じ場所に集める)",
    ]
    return 新しい定義, 変更点
...
def main() -> int:
    ...
    引数の解析.add_argument(
        "--監視先を変える",
        action="store_true",
        help="改造済みのフロー①を別のTeamsチーム(別サイト)向けに作り替える",
    )
    引数の解析.add_argument("--サイトurl", help="--監視先を変える のときに指定する")
    引数の解析.add_argument(
        "--ライブラリ", help="--監視先を変える のときに指定する(ドキュメントライブラリのID)"
    )
    引数 = 引数の解析.parse_args()
    ...
    if 引数.監視先を変える:
        if not (引数.サイトurl and 引数.ライブラリ):
            raise SystemExit("--監視先を変える には --サイトurl と --ライブラリ が必要です")
        新しい定義, 変更点 = 監視先を差し替える(
            元の定義,
            サイトurl=引数.サイトurl,
            ライブラリ=引数.ライブラリ,
