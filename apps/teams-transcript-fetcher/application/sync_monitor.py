...
「Macは起きているのに同期だけが止まる」事態を、Power Automateが5分間隔で書く
...
class 通知事象:
    ...
    まで次サイクル以降も再試行される(監視記録の保留中の再起動通知で管理)。
...
class 監視記録:
    ...
    異常終了の連続回数: int = 0
    #: 「同期停滞を検知して再起動した」の通知で、まだ書き出しに成功していないもの。
    #: 停滞イベントの生死とは独立に持つ(design.md#監視通知の書き出しバッチサイクルの最後
    #: 手順1・5: 停滞イベントが解消した後でも、書き出しに成功するまで再試行する)。
    保留中の再起動通知: 通知事象 | None = None
...
def 読み込む(監視記録ファイル: Path) -> 監視記録:
    ...
    記録が失われると回数制限・通知抑止の記憶も失われるが、上限は24時間4回に
    限られるため許容する(design.md#エラーハンドリング)。
    """
    try:
    ...
    保留中の中身 = 中身.get("pending_restart_notice")
    保留中 = None
    if isinstance(保留中の中身, dict):
        検知時刻 = _日時を読む(保留中の中身.get("detected_at"))
        種別 = 保留中の中身.get("kind")
        キー = 保留中の中身.get("key")
        要点 = 保留中の中身.get("summary")
        if (
            検知時刻 is not None
            and isinstance(種別, str) and 種別
            and isinstance(キー, str) and キー
            and isinstance(要点, str) and 要点
        ):
            保留中 = 通知事象(
                種別=種別, キー=キー, 検知時刻=検知時刻, 要点=要点, 即時=True
            )
...
def 保存する(記録: 監視記録, 監視記録ファイル: Path, 現在時刻: datetime) -> None:
    ...
    書き出す中身 = {
        ...
        "pending_restart_notice": (
            {
                "kind": 記録.保留中の再起動通知.種別,
                "key": 記録.保留中の再起動通知.キー,
                "detected_at": 記録.保留中の再起動通知.検知時刻.isoformat(),
                "summary": 記録.保留中の再起動通知.要点,
            }
            if 記録.保留中の再起動通知
            else None
        ),
...
def 停滞を判定する(
    ...
    未書き出しの再起動通知(`記録.保留中の再起動通知`)は、内部でどの分岐を通った
    かに関わらずここで一律に積む。書き出しに成功するまで(`通知を評価する` が
    Noneに戻すまで)次サイクル以降も再試行される
    (design.md#監視通知の書き出しバッチサイクルの最後 手順1・5)。
    """
    if 疎通確認 is None:
    ...
    try:
        事象たち = _停滞を判定する(記録, 設定, 現在時刻, 疎通確認, 再起動する)
    finally:
        記録.前回実行時刻 = 現在時刻

    if 記録.保留中の再起動通知 is not None:
        事象たち.append(記録.保留中の再起動通知)
    return 事象たち
...
def _復旧を進める(
    ...
    「同期停滞を検知して再起動した」の即時通知は `記録.保留中の再起動通知` に
    持たせる。呼び出し元(公開関数`停滞を判定する`)が、この関数がどの分岐を
    返しても一律に積み直すため、ここでは再起動時に設定するだけでよい
    (再起動そのものはイベントにつき1回に留める。requirements.md#再起動の回数制限)。
    """
    イベント = 記録.停滞イベント
    ...
    if イベント.復旧失敗判定済み:
        ...
        return [復旧失敗の事象(f"復旧失敗が継続中(鮮度={鮮度の表示})")]
    ...
    if 記録.保留中の再起動通知 is not None:
        # 前の停滞イベントの再起動通知がまだ書き出せていない状態で、24時間
        # 上限内の別の停滞が再起動に至った。1件しか保持できないため、前回分は
        # ここで失われる(design.md#監視通知の書き出しバッチサイクルの最後)。
        logger.warning(
            "未書き出しの再起動通知を新しい停滞の通知で置き換える(前回分は失われる): %s",
            記録.保留中の再起動通知.要点,
        )
    記録.保留中の再起動通知 = 通知事象(
        種別=事象_同期停滞,
...
def 通知を評価する(
    ...
    即時の事象は常に書き出しを試みる。成功した「同期停滞」の事象は
    `記録.保留中の再起動通知` をNoneに戻し、以後同じ再起動については積まれなく
    なる(失敗した場合は`停滞を判定する`が次サイクルでも積み直すため、ここでは
    何もしない)。継続型は通知済み記録と突き合わせ、初回と
    24時間経過のみ書き出す。継続していない事象キーは記録から取り除き、解消後の
    再発を新しい事象として通知できるようにする。
    """
    if 書き出す is None:
