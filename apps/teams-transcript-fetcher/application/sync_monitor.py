...
def 読み込む(監視記録ファイル: Path) -> 監視記録:
    ...
    if isinstance(保留中の中身, dict):
        ...
        if 検知時刻 is not None and 種別 and キー and 要点:
            保留中 = 通知事象(
...
def 保留中の再起動通知を積む(
    記録: 監視記録, 事象たち: list[通知事象]
) -> list[通知事象]:
    """未書き出しの再起動通知(あれば)を積む。書き出しに成功するまで
    (`通知を評価する` が `記録.保留中の再起動通知` を None にするまで)呼び出す
    たびに積み直される(design.md#監視通知の書き出しバッチサイクルの最後 手順1・5)。
    """
...
def _停滞を判定する(
    ...
    if 鮮度 <= timedelta(minutes=設定.停滞判定しきい値分):
        # 保留中の再起動通知は停滞イベントの生死と独立に持つため、イベントが
        # 解消してもここで積み続け、書き出しに成功するまで次サイクル以降も
        # 再試行される(design.md#監視通知の書き出しバッチサイクルの最後 手順1・5)。
        事象たち: list[通知事象] = 保留中の再起動通知を積む(記録, [])
...
def _復旧を進める(
    ...
    持たせ、書き出しに成功するまで(`通知を評価する` がNoneに戻すまで)ここで
    積み続け、次サイクル以降も再試行する。再起動そのものはイベントにつき1回に
    留める(requirements.md#再起動の回数制限)。
    """
    イベント = 記録.停滞イベント
    ...
    if イベント.復旧失敗判定済み:
        ...
        return 保留中の再起動通知を積む(
            記録, [復旧失敗の事象(f"復旧失敗が継続中(鮮度={鮮度の表示})")]
        )
...
def 通知を評価する(
    ...
    なる(失敗した場合は呼び出し元(保留中の再起動通知を積む)が次サイクルでも
    積み直すため、ここでは何もしない)。継続型は通知済み記録と突き合わせ、初回と
