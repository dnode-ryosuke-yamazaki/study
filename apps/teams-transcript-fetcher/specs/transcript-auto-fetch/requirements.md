...
会議の参加者が、Teams会議のトランスクリプト(WEBVTT)を手作業でダウンロードすることなくOneDriveへ自動蓄積できるようにする。Power Automateが録画1件ごとに台帳を作り、ローカルMac上のバッチが台帳の到着を検知して未取得のトランスクリプトを取得して保存する(5分間隔の定期起動を取りこぼしの回収に併用する)。M365へのAPI認証は行わず、OneDrive同期クライアントに委ねる。
...
- [7] 台帳置き場(`ledger/`)とURL置き場(`url/`)にファイルが届いた時点でバッチが起動すること。定期起動(5分間隔)はこれと併用し、取りこぼしの回収に用いること — 根拠: 台帳が届いてから取得を始めるまでの待ちを定期起動の間隔から切り離す。イベント起動だけに寄せると、スリープ中に届いたファイルやイベントの取りこぼしが回収されないため定期起動を残す

## 非機能要件

- 会議終了からトランスクリプト保存までの遅延は、**約4〜12分**を許容する
  - 内訳: Teams側のトランスクリプト生成(約3分)+ 録画作成の検知(フロー①のトリガー。1〜3分)+ 台帳のOneDrive同期(0〜5分)+ 台帳の到着によるバッチの起動(即時)+ ダウンロードと保存(数秒〜1分)
  - ダウンロードURLの再発行を経る場合はこれに、要求の同期 + 発行の検知(最大1分)+ URLの同期 + URLの到着によるバッチの起動(即時)が加わる
  - **上記はいずれも「録画の作成時点でトランスクリプトが生成済みである」場合の値。** 生成が遅れた場合は、URLの再発行を経て遅れの分だけ後ろにずれる
  - **OneDrive同期が停滞している間はこの範囲に収まらない。** 停滞の検知と自動復旧は [sync-stall-recovery](../sync-stall-recovery/requirements.md) が担うが、検知と復旧に要する時間の分は必ず超過する
