...
| # | 項目 | 内容と影響 | 状態 |
|---|---|---|---|
| 1 | ダウンロードURLの寿命 | **実機で確認済**(2026-08-19)。発行から**9.7分後でも取得に成功**した。フェーズ2のハンドシェイク(約10分)は成立する見込み。上限は未測定だが、設計はログで観測し続けられる形になっている | **確認済** |
...
| 11 | ダウンロードURLのホスト名 | **実機で確認済**(2026-08-19)。`jpdeloitte.sharepoint.com` で、暫定の許可リスト(`.sharepoint.com`)にそのまま合致した。設定の追加は不要 | **確認済** |
