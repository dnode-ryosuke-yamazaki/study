...
- [1] 未取得の録画のうち、有効なダウンロードURLを持たないもの、**または既知件数分のURLが揃わないもの**について要求が作成されること
...
| 11 | ダウンロードURLのホスト名 | アクセス先URLの検証([design.md#セキュリティ]で要求)に使う許可リストの内容。実際のホストが未確認のため、暫定で `.sharepoint.com` と `.svc.ms` を許可している。**拒否した場合はホスト名をログに残すため、初回の実機実行で判明する。** 想定外のホストだった場合は許可リストの設定値を追加する(コード変更は不要) | 実機で観測 |
