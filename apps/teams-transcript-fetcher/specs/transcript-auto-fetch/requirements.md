...
- [1] 未取得の録画のうち、有効なダウンロードURLを持たないものについて要求が作成されること
...
- [2] 応答が成功であっても本文がWEBVTT形式でない場合、失敗として扱い保存しないこと — 根拠: 期限切れのURLがエラーページを本文として返し、ステータスだけは成功になる場合がある。保存してしまうと取得済みとして記録され台帳も削除されるため、復旧できない誤りになる
...
- [5] 一覧は追記順で安定している(すでに存在するトランスクリプトの並び順が後から変わらず、**一覧から消えることもない**)ことを前提とすること — 根拠: トランスクリプトは会議の分割・再開によって追加される想定であるため。この前提が崩れると別のトランスクリプトを取得済みと誤認する、または既知件数が永久に揃わない。**「消えないこと」は観測できるが、「並び順が変わらないこと」は完全には観測できず、一部を受容する**([前提・検証項目](#前提検証項目) #8・#8')
...
- [4] バッチが読むフォルダはOneDriveの「常にこのデバイス上に保持」で固定されている前提とすること — 根拠: Files On-Demandの「オンラインのみ」状態ではオフライン時に読み取れない
- [5] SharePointライブラリの新規同期設定を必要としないこと — 根拠: サイト単位の同期がポリシーで禁止されている可能性があり、成立性を外部要因に依存させない
- [6] 作業フォルダを `00_root/auto/` の直下に置かないこと — 根拠: `00_root/auto/` 直下のファイル作成はTeams投稿用のPower Automateフローが検知するため、意図しない投稿が発生する。同フローのトリガーは `00_root/auto/teamsNotice/` のみを対象とするよう修正済みで、`00_root/auto/transcript/` 配下では起動しない(検証項目9)
...
| # | 項目 | 内容と影響 | 状態 |
|---|---|---|---|
| 1 | ダウンロードURLの寿命 | フェーズ2の成立には「URLの寿命 > 要求から消費までのレイテンシ(約10分)」が必要。**事前実測は行わず、URLの発行時刻から取得試行までの経過時間を成功・失敗ともにログに記録することで、運用の中で境界が蓄積されるようにする。** 期限切れが繰り返し観測された場合は実行間隔を短縮する | 実機で観測 |
...
| 3 | フローの改造方法 | フローのエクスポート/インポートによる改造が可能かを確認する。できない場合は手順書によるUI操作に切り替える | 未確認 |
| 4 | 台帳に含められる情報 | 既存フローのトリガーから、会議名・録画の格納先・録画のファイル識別子・録画の作成日時が取得できることを確認した。会議の開始時刻そのものは取得できない | **確認済** |
...
| 8 | 一覧の並び順と件数の安定性 | 状態管理 [5] の前提。**②件数が減ることがないかは「選んだ一覧のURLの数」の推移をログで直接確認する**(既知件数は下限で固定されるため、そちらでは観測できない)。崩れていた場合は既知件数の下限(列挙 [5])の扱いを見直す。暫定的な打ち切りはエラー時の挙動 [7] で用意してある | ②は実機で観測 |
| 8' | 一覧の並び順の保存(①) | **直接観測できず、間接検知にも限界がある。** 保存済みのトランスクリプトを再取得しないため、同一性を比較する材料がログに残らない。①の破れ方は2つあり、**件数が増える形で現れた場合は「件数は増えているのに保存が進まない」症状としてエラー時の挙動 [7] で検知できるが、件数が変わらないまま並び順だけ入れ替わった場合は検知できない**(全インデックスが取得済みと一致し、その録画は全件保存済みと判定されて台帳が削除される)。後者は**前提として受容する**。検証項目2で一覧の応答に一意な識別子が含まれると分かった場合は、その値をログに出して直接検知できるようにし、識別方式もその識別子へ切り替える | 一部のみ間接検知。**残りは受容** |
| 9 | Teams投稿用フローの監視範囲 | 実行環境 [6] の前提。Teams投稿用のPower Automateフローのトリガーを `00_root/auto/teamsNotice/` のみを対象とするよう修正済みであり、`00_root/auto/transcript/` 配下のファイル作成では起動しない | **確認済** |
| 10 | 録画のファイル識別子がファイル名に使える形式か | 台帳とURLファイルの名前は録画のファイル識別子をそのまま使う(design.md の識別子とファイル名の規則)。この値が「ファイル名 [2]」と同じ制約(使えない文字を含まない)を満たすかは未確認。**満たさない場合はフロー①が台帳を作れずフェーズ1が丸ごと動かないため、最初のフロー改造時に実際に作成できることを確認する。** 満たさない場合は、識別子を安全な文字列へ変換する規則を「識別子とファイル名の規則」に追加する(3者で共有するため単純な置換規則に限る) | 未確認 |
...
| 通常会議用フローが録画の格納先の識別子をハードコードしており、そのために実行している一覧取得の結果を使っていない | 今回の改造で、トリガーが提供する値を使う方式に変更する(チャネル会議用フローは既にこの方式)。ハードコードした値と不要な呼び出し2件がなくなる |
