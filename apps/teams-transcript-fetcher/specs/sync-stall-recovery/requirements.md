...
- [1] Power Automateの定期フローが、5分間隔で現在時刻をハートビートファイルに書き込むこと。根拠: 停滞を検知できるまでの時間はハートビート間隔に比例するため、取得バッチの起動間隔と同じ5分に揃える
...
- [2] 鮮度が閾値(20分)を超えており、かつネットワークが生きている場合に限り、同期停滞と判定すること
- [3] ネットワークが不通の場合は同期停滞と判定せず、再起動も停滞の通知も行わないこと
- [4] スリープ復帰直後は同期停滞と判定しないこと(判定条件は[ビジネスルール](#スリープ復帰直後の誤検知防止)を参照)
- [5] ハートビートファイルが正常に読めない場合の挙動は[ビジネスルール](#ハートビートの異常値の扱い)に従うこと
- [6] 検知の判定結果と根拠(鮮度・ネットワーク状態)がログに残り、後から確認できること
...
- [1] ハートビートの鮮度が20分を超えた場合、停滞の候補と判定すること。根拠: ハートビート4回分の欠落に相当する。正常時の鮮度はハートビート間隔(5分)に同期の通常ラグ(数分)が乗る範囲に収まるため、20分ならPower Automateの実行遅延や単発の書き込み失敗では誤検知しない。判定は鮮度の絶対値で行うため、閾値を下回っている間は更新が完全に止まっていても停滞と判定できない。この無検知の時間は上位の目標(会議終了から15分以内の通知)をそのまま押し下げるので、誤検知しない範囲で最小の値を採る
- [2] ネットワークが不通の間は、鮮度によらず停滞と判定しないこと。根拠: 同期停滞はネットワークが生きているのにファイルが届かない状態を指し、ネットワーク断はOneDrive再起動では直らない別の異常であるため

### ハートビートの異常値の扱い

- [1] ハートビートファイルが存在しない・読めない・時刻として解釈できない場合は、同期停滞と判定しないこと(再起動も停滞の通知も行わない)。根拠: ハートビートフローが未構築・故障している期間に、正常に動いている同期クライアントを誤って再起動しないため。安全側(再起動しない側)に倒す
- [2] 上記の間は停滞検知が働かないため、警告ログを残して後から確認できるようにすること
- [3] 記載された時刻が現在時刻より未来の場合(時計ずれ)、鮮度は0とみなすこと。根拠: 停滞ではない側に倒し、時計ずれによる誤った再起動を防ぐ

### スリープ復帰直後の誤検知防止

- [1] 前回のバッチ実行から15分を超えて間隔が空いていた場合、スリープ等による実行の中断とみなすこと。根拠: バッチは5分間隔で起動されるため、3サイクル分を超える空白は稼働の連続性が切れた証拠になる
- [2] 実行の中断からの復帰後20分間は、同期停滞と判定しないこと。根拠: 復帰直後はハートビートが古いのが正常であり、新しいハートビートが書かれて同期されるまで(ハートビート間隔+同期ラグ)の猶予が必要。値は[停滞判定の閾値](#停滞判定の閾値)(20分)と揃え、猶予が閾値より短くなって復帰直後の判定が実質的に素通りになることを防ぐ

### 再起動の回数制限

- [1] 再起動は同一の停滞イベントにつき1回までとすること。根拠: 1回で直らない停滞は再起動の繰り返しでは解決せず、人の対応が必要
- [2] 同一の停滞イベントとは、停滞判定が始まってからハートビートの鮮度が閾値内へ戻るまでの期間を指すこと
- [3] 直近24時間の再起動は合計4回までとすること。根拠: 短時間に停滞判定と復旧を繰り返す状態で再起動がループすることを防ぐ。一方で、1日のうちに互いに独立した停滞が複数回起きることは実際にあり(半日のうちに2回起きた実測がある)、上限が小さすぎると2回目以降の停滞で自動復旧できず人の対応を待つことになる
- [4] 再起動後45分以内にハートビートの鮮度が閾値内へ戻らない場合、復旧失敗と判定すること。根拠: 再起動から鮮度が閾値内へ戻るまでに35分を要した実測があり、これより短いと復旧しつつある状態を復旧失敗として通知してしまう。この変更で真の復旧失敗の通知も最大45分遅れるが、通知は人が対応を始める合図であって復旧の手段ではなく、遅れによって取得が止まる時間そのものが伸びるわけではないため許容する

### 失敗・警告の連続の通知

- [1] 同期停滞以外でも、同一の失敗・警告が3回連続した場合に通知すること。対象は既存バッチが記録している異常(同一録画のダウンロード失敗の連続、台帳の読み取り失敗の持ち越しの連続、URL読み取り待ちの持ち越しの連続、バッチ自体の異常終了の連続)とすること。根拠: 3サイクル続くことを条件にすれば、一過性の失敗を除外できる。既存の「読めない台帳・URLファイルは3回連続で記録」の閾値とも揃う。定期起動だけなら約15分に相当するが、即時起動を併用するため実時間はこれより短くなりうる
- [2] 「発行要求を出しても保存が進まない実行が10回続いた録画」([transcript-auto-fetch/requirements.md#エラー時の挙動](../transcript-auto-fetch/requirements.md#エラー時の挙動) [7])による要手動確認は、通知の対象外とすること。根拠: 大半が文字起こしを有効にしていない会議による日常的な事象であり、通知するとノイズになるため
...
- [4] OneDrive再起動の直後は同期経路がまだ使えず、通知ファイルの書き出し自体が一時的に失敗することがある(制約として許容する)。書き出しに成功するまで次サイクル以降も再試行すること

### 記録

- [1] 停滞イベント・再起動・通知の履歴は、回数制限(24時間で4回)と再通知の抑止(24時間ごと)の判定に使える形で保持すること
...
- ハートビートを書き込むPower Automateの定期フロー(5分間隔)が稼働していること
