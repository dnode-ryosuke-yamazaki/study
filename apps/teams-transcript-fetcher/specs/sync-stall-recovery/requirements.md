...
- [1] ハートビートファイルが存在しない・読めない・時刻として解釈できない場合は、同期停滞と判定しないこと(再起動も停滞の通知も行わない)。根拠: ハートビートフローが未構築・故障している期間に、正常に動いている同期クライアントを誤って再起動しないため。安全側(再起動しない側)に倒す- [2] 上記の間は停滞検知が働かないため、警告ログを残して後から確認できるようにすること- [3] 記載された時刻が現在時刻より未来の場合(時計ずれ)、鮮度は0とみなすこと。根拠: 停滞ではない側に倒し、時計ずれによる誤った再起動を防ぐ
### スリープ復帰直後の誤検知防止
