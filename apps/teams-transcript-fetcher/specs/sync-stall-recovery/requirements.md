...
- [1] ハートビートファイルが存在しない・読めない・時刻として解釈できない場合は、同期停滞と判定しないこと(再起動も停滞の通知も行わない)。根拠: ハートビートフローが未構築・故障している期間に、正常に動いている同期クライアントを誤って再起動しないため。安全側(再起動しない側)に倒す【推測】
- [2] 上記の間は停滞検知が働かないため、警告ログを残して後から確認できるようにすること【推測】
- [3] 記載された時刻が現在時刻より未来の場合(時計ずれ)、鮮度は0とみなすこと。根拠: 停滞ではない側に倒し、時計ずれによる誤った再起動を防ぐ【推測】

### スリープ復帰直後の誤検知防止

- [1] 前回のバッチ実行から15分を超えて間隔が空いていた場合、スリープ等による実行の中断とみなすこと。根拠: バッチは5分間隔で起動されるため、3サイクル分を超える空白は稼働の連続性が切れた証拠になる
- [2] 実行の中断からの復帰後45分間は、同期停滞と判定しないこと。根拠: 復帰直後はハートビートが古いのが正常であり、新しいハートビートが書かれて同期されるまで(ハートビート間隔+同期ラグ)の猶予が必要。値は[停滞判定の閾値](#停滞判定の閾値)(45分)と揃え、猶予が閾値より短くなって復帰直後の判定が実質的に素通りになることを防ぐ【推測】
...
- [2] 「発行要求を出しても保存が進まない実行が10回続いた録画」([transcript-auto-fetch/requirements.md#エラー時の挙動](../transcript-auto-fetch/requirements.md#エラー時の挙動) [7])による要手動確認は、通知の対象外とすること。根拠: 大半が文字起こしを有効にしていない会議による日常的な事象であり、通知するとノイズになるため【推測】
