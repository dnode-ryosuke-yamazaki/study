...
**しきい値の変更を含む更新では、先に [反映の順序](application/power-automate/README.md#反映の順序既に15分間隔で動いているフローを切り替える場合) を読んでください。** ハートビートフローの周期を変える前にバッチ側のしきい値を配ると、正常なOneDriveを停滞と誤判定します。
