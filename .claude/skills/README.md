...
1. **入口**: UserPromptSubmitフック(`../hooks/route-to-workflow.sh`)がユーザー入力に「開発作業なら該当する工程Skillを起動してから作業する」という指示を注入する(工程の判定はモデルが行う。開発ワードを1語でも含む入力には常に注入し、含まない場合のみ挨拶・単純な質問を注入対象から外す。`/`で始まる明示的なSkill起動には注入しない)
